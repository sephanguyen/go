...
CREATE STREAM IF NOT EXISTS USERS_STREAM_ORIGIN_V3   WITH (kafka_topic='local.manabie.bob.public.users', value_format='AVRO');
...
CREATE  STREAM IF NOT EXISTS STUDENT_STREAM_ORIGIN_V3  WITH (kafka_topic='local.manabie.bob.public.students', value_format='AVRO');
...
CREATE SINK CONNECTOR SINK_BASIC_USER_INFO_NEW_V5 WITH (
      'connector.class'='io.confluent.connect.jdbc.JdbcSinkConnector',
      'transforms.unwrap.delete.handling.mode'='drop',
      'tasks.max'='1',
      'topics'='STUDENT_PUBLIC_INFO_V3',
      ...
      'transforms.unwrap.drop.tombstones'='true',
      'auto.create'='false',
      'connection.url'='${file:/decrypted/kafka-connect.secrets.properties:alloydb_url}',
      'insert.mode'='upsert',
      'pk.mode'='record_key',
      'transforms'='RenameField',
      'transforms.RenameField.type'= 'org.apache.kafka.connect.transforms.ReplaceField$Value',
