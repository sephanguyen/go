...
DROP CONNECTOR IF EXISTS SINK_TAGGED_USER_PUBLIC_INFO_V1;
CREATE SINK CONNECTOR IF NOT EXISTS SINK_TAGGED_USER_PUBLIC_INFO_V1 WITH (
      ...
      'insert.mode'='upsert',
      'table.name.format'='public.tagged_user',
...
DROP CONNECTOR IF EXISTS SINK_USER_GROUP_MEMBER_PUBLIC_INFO_V1;
CREATE SINK CONNECTOR IF NOT EXISTS SINK_USER_GROUP_MEMBER_PUBLIC_INFO_V1 WITH (
      ...
      'connection.url'='${file:/decrypted/kafka-connect.secrets.properties:kec_url}',
      'insert.mode'='upsert',
      'table.name.format'='public.user_group_member',
      'pk.mode'='record_value',
      'transforms'='RenameField',
      'transforms.RenameField.type'= 'org.apache.kafka.connect.transforms.ReplaceField$Value',
...
DROP CONNECTOR IF EXISTS SINK_SCHOOL_HISTORY_PUBLIC_INFO_V1;
CREATE SINK CONNECTOR IF NOT EXISTS SINK_SCHOOL_HISTORY_PUBLIC_INFO_V1 WITH (
      ...
      'connection.url'='${file:/decrypted/kafka-connect.secrets.properties:kec_url}',
      'insert.mode'='upsert',
      'table.name.format'='public.school_history',
      'pk.mode'='record_value',
      'transforms'='RenameField',
      'transforms.RenameField.type'= 'org.apache.kafka.connect.transforms.ReplaceField$Value',
...
DROP CONNECTOR IF EXISTS SINK_STUDENT_ENROLLMENT_STATUS_HISTORY_PUBLIC_INFO_V1;
CREATE SINK CONNECTOR IF NOT EXISTS SINK_STUDENT_ENROLLMENT_STATUS_HISTORY_PUBLIC_INFO_V1 WITH (
      ...
      'connection.url'='${file:/decrypted/kafka-connect.secrets.properties:kec_url}',
      'insert.mode'='upsert',
      'table.name.format'='public.student_enrollment_status_history',
      'pk.mode'='record_value',
      'transforms'='RenameField',
      'transforms.RenameField.type'= 'org.apache.kafka.connect.transforms.ReplaceField$Value',
...
DROP CONNECTOR IF EXISTS SINK_PERMISSION_PUBLIC_INFO_V1;
CREATE SINK CONNECTOR IF NOT EXISTS SINK_PERMISSION_PUBLIC_INFO_V1 WITH (
      'connector.class'='io.confluent.connect.jdbc.JdbcSinkConnector',
      'transforms.unwrap.delete.handling.mode'='drop',
      'tasks.max'='1',
      ...
      'connection.url'='${file:/decrypted/kafka-connect.secrets.properties:kec_url}',
      'insert.mode'='upsert',
      'table.name.format'='public.permission',
      'pk.mode'='record_value',
      'transforms'='RenameField',
      'transforms.RenameField.type'= 'org.apache.kafka.connect.transforms.ReplaceField$Value',
      ...
      'pk.fields'='permission_id,user_group_id,granted_permission_role_id,location_id'
);
DROP CONNECTOR IF EXISTS SINK_PERMISSION_STREAM_FORMATED_V1;
CREATE SINK CONNECTOR IF NOT EXISTS SINK_PERMISSION_STREAM_FORMATED_V1 WITH (
      'connector.class'='io.confluent.connect.jdbc.JdbcSinkConnector',
      'transforms.unwrap.delete.handling.mode'='drop',
      'tasks.max'='1',
      'topics'='{{ .Values.topicPrefix }}PERMISSION_STREAM_FORMATED_V1',
      'fields.whitelist'='user_group_id,granted_permission_role_id,location_id,permission_id,permission_created_at,permission_updated_at,permission_deleted_at,permission_permission_name',
      'key.converter'='org.apache.kafka.connect.storage.StringConverter',
      'value.converter'='io.confluent.connect.avro.AvroConverter',
      'value.converter.schema.registry.url'='{{ .Values.cpRegistryHost }}',
      'delete.enabled'='false',
      ...
      'transforms'='RenameField',
      'transforms.RenameField.type'= 'org.apache.kafka.connect.transforms.ReplaceField$Value',
      'transforms.RenameField.renames'='USER_GROUP_ID:user_group_id,GRANTED_PERMISSION_ROLE_ID:granted_permission_role_id,LOCATION_ID:location_id,PERMISSION_ID:permission_id,CREATED_AT:permission_created_at,UPDATED_AT:permission_updated_at,DELETED_AT:permission_deleted_at,PERMISSION_NAME:permission_permission_name',
      'pk.fields'='permission_id,user_group_id,granted_permission_role_id,location_id'
);


DROP CONNECTOR IF EXISTS SINK_SCHOOL_LEVEL_PUBLIC_INFO_V1;
CREATE SINK CONNECTOR IF NOT EXISTS SINK_SCHOOL_LEVEL_PUBLIC_INFO_V1 WITH (
      ...
      'connection.url'='${file:/decrypted/kafka-connect.secrets.properties:kec_url}',
      'insert.mode'='upsert',
      'table.name.format'='public.school_level',
      'pk.mode'='record_value',
      'transforms'='RenameField',
      'transforms.RenameField.type'= 'org.apache.kafka.connect.transforms.ReplaceField$Value',
...
DROP CONNECTOR IF EXISTS SINK_SCHOOL_LEVEL_STREAM_FORMATED_V1;
...
DROP CONNECTOR IF EXISTS SINK_SCHOOL_COURSE_SCHOOL_INFO_PUBLIC_INFO_V1;
CREATE SINK CONNECTOR IF NOT EXISTS SINK_SCHOOL_COURSE_SCHOOL_INFO_PUBLIC_INFO_V1 WITH (
      ...
      'connection.url'='${file:/decrypted/kafka-connect.secrets.properties:kec_url}',
      'insert.mode'='upsert',
      'table.name.format'='public.school_course_school_info',
      'pk.mode'='record_value',
      'transforms'='RenameField',
      'transforms.RenameField.type'= 'org.apache.kafka.connect.transforms.ReplaceField$Value',
...
DROP CONNECTOR IF EXISTS SINK_SCHOOL_INFO_STREAM_FORMATED_V1;
...
DROP CONNECTOR IF EXISTS SINK_USER_ADDRESS_PUBLIC_INFO_V1;
CREATE SINK CONNECTOR IF NOT EXISTS SINK_USER_ADDRESS_PUBLIC_INFO_V1 WITH (
      ...
      'connection.url'='${file:/decrypted/kafka-connect.secrets.properties:kec_url}',
      'insert.mode'='upsert',
      'table.name.format'='public.user_address',
      'pk.mode'='record_key',
      'transforms'='RenameField',
      'transforms.RenameField.type'= 'org.apache.kafka.connect.transforms.ReplaceField$Value',
...
DROP CONNECTOR IF EXISTS SINK_USER_PHONE_NUMBER_PUBLIC_INFO_V1;
CREATE SINK CONNECTOR IF NOT EXISTS SINK_USER_PHONE_NUMBER_PUBLIC_INFO_V1 WITH (
      ...
      'connection.url'='${file:/decrypted/kafka-connect.secrets.properties:kec_url}',
      'insert.mode'='upsert',
      'table.name.format'='public.user_phone_number',
      'pk.mode'='record_key',
      'transforms'='RenameField',
      'transforms.RenameField.type'= 'org.apache.kafka.connect.transforms.ReplaceField$Value',
...
DROP CONNECTOR IF EXISTS SINK_USER_GROUP_PUBLIC_INFO_V1;
CREATE SINK CONNECTOR IF NOT EXISTS SINK_USER_GROUP_PUBLIC_INFO_V1 WITH (
      ...
      'connection.url'='${file:/decrypted/kafka-connect.secrets.properties:kec_url}',
      'insert.mode'='upsert',
      'table.name.format'='public.user_group',
      'pk.mode'='record_key',
      'transforms'='RenameField',
      'transforms.RenameField.type'= 'org.apache.kafka.connect.transforms.ReplaceField$Value',
...
DROP CONNECTOR IF EXISTS SINK_ROLE_PUBLIC_INFO_V1;
...
DROP CONNECTOR IF EXISTS SINK_ROLE_STREAM_FORMATED_V1;
...
CREATE SINK CONNECTOR IF NOT EXISTS SINK_PARENTS_PUBLIC_INFO_V1 WITH (
      ...
      'connection.url'='${file:/decrypted/kafka-connect.secrets.properties:kec_url}',
      'insert.mode'='upsert',
      'table.name.format'='public.parents',
      'pk.mode'='record_key',
      'transforms'='RenameField',
      'transforms.RenameField.type'= 'org.apache.kafka.connect.transforms.ReplaceField$Value',
...
CREATE SINK CONNECTOR IF NOT EXISTS SINK_STUDENTS_PUBLIC_INFO_V1 WITH (
      ...
      'connection.url'='${file:/decrypted/kafka-connect.secrets.properties:kec_url}',
      'insert.mode'='upsert',
      'table.name.format'='public.students',
      'pk.mode'='record_key',
      'transforms'='RenameField',
      'transforms.RenameField.type'= 'org.apache.kafka.connect.transforms.ReplaceField$Value',
...
CREATE SINK CONNECTOR IF NOT EXISTS SINK_STAFF_PUBLIC_INFO_V1 WITH (
      ...
      'connection.url'='${file:/decrypted/kafka-connect.secrets.properties:kec_url}',
      'insert.mode'='upsert',
      'table.name.format'='public.staff',
      'pk.mode'='record_key',
      'transforms'='RenameField',
      'transforms.RenameField.type'= 'org.apache.kafka.connect.transforms.ReplaceField$Value',
...
DROP CONNECTOR IF EXISTS SINK_STUDENT_PARENT_PUBLIC_INFO_V1;
CREATE SINK CONNECTOR IF NOT EXISTS SINK_STUDENT_PARENT_PUBLIC_INFO_V1 WITH (
      ...
      'connection.url'='${file:/decrypted/kafka-connect.secrets.properties:kec_url}',
      'insert.mode'='upsert',
      'table.name.format'='public.student_parent',
      'pk.mode'='record_value',
      'transforms'='RenameField',
      'transforms.RenameField.type'= 'org.apache.kafka.connect.transforms.ReplaceField$Value',
