locals {
  ...
  service_definitions = yamldecode(file("${get_repo_root()}/deployments/decl/stag-defs.yaml"))
  ...
  postgresql_user_permissions = flatten(concat(
    ...
    # Grant j4 read only access to service having j4.enabled=true
    [
      for service in local.service_definitions : [
        {
          database    = "${local.db_prefix}${service.name}"
          user        = "${local.service_account_prefix}j4@${local.project_id}.iam"
          owner       = "${local.service_account_prefix}${service.name}-m@${local.project_id}.iam"
          schema      = "public"
          object_type = "table"
          privileges  = ["SELECT"]
      ...
      ] if try(service.postgresql.createdb, false) && try(service.j4.allow_db_access, false)
    ],
  ...
  postgresql_replication_roles = [
    ...
    "${local.service_account_prefix}dwh-kafka-connect@${local.project_id}.iam",
  ]
  ...
  service_accounts = flatten(concat(
    ...
    [
      ...
      },
      {
        ...
        # local.runs_on_project_id, that's because for Staging Manabie,
        # both Firebase project and Identity Platform project are
        # using the the same staging-manabie-online project.
        project             = local.project_id
        roles               = {}
        identity_namespaces = []
      },
      {
        ...
        roles = {
          "${local.runs_on_project_id}" = [
            "roles/artifactregistry.reader", # allow adhoc to pull custom image for cloud build
...
inputs = {
  ...
  bucket_name = dependency.platforms_storage.outputs.backend_bucket
