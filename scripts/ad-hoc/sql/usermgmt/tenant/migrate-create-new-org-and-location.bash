...
VALUES	('${LOCATION_ID}','org','${ORG_NAME}', '${ORG_ID}', now(), now()) ON CONFLICT DO NOTHING;
...
VALUES	('${LOCATION_ID}', '${ORG_NAME}','${LOCATION_ID}',NULL, NULL, NULL, '${ORG_ID}', now(), now(),'${LOCATION_ID}') ON CONFLICT DO NOTHING;

INSERT INTO public.organization_auths
(organization_id, auth_project_id, auth_tenant_id)
VALUES(${ORG_ID}, '${AUTH_PROJECT_ID}', '${TENANT_ID}') ON CONFLICT DO NOTHING;
...
VALUES  (${ORG_ID}, '${ORG_NAME}', 'COUNTRY_JP', 1, 1, null, false, now(), now(), false,null, null,'${ORG_ID}') ON CONFLICT DO NOTHING;
