package notificationmgmt

import (
	"context"
	"fmt"

	"github.com/manabie-com/backend/internal/golibs/bootstrap"
	"github.com/manabie-com/backend/internal/golibs/database"
	"github.com/manabie-com/backend/internal/golibs/logger"
	"github.com/manabie-com/backend/internal/notification/config"
	"github.com/manabie-com/backend/internal/notification/entities"

	"github.com/jackc/pgx/v4"
	"github.com/jackc/pgx/v4/pgxpool"
	"go.uber.org/zap"
)

func init() {
	bootstrap.RegisterJob("notificationmgmt_migrate_notification_class_filter", RunMigrateNotificationClassFilter)
}

func RunMigrateNotificationClassFilter(ctx context.Context, notiCfg config.Config, rsc *bootstrap.Resources) error {
	db := rsc.DBWith("bob")
	zLogger := rsc.Logger()

	return MigrateNotificationClassFilter(ctx, notiCfg, db.DB.(*pgxpool.Pool), zLogger)
}

func MigrateNotificationClassFilter(ctx context.Context, notiCfg config.Config, dbPool *pgxpool.Pool, zLogger *zap.Logger) error {
	if zLogger == nil {
		zLogger = logger.NewZapLogger("debug", notiCfg.Common.Environment == localEnv)
	}

	organizations, err := dbPool.Query(ctx, scanOrganiationQuery)
	if err != nil {
		return fmt.Errorf("get orgs failed: %s", err)
	}
	defer organizations.Close()

	// Migrate with RP
	totalNotiProcessed := 0
	for organizations.Next() {
		var organizationID string
		err := organizations.Scan(&organizationID)
		if err != nil {
			return fmt.Errorf("failed to scan an orgs row: %w", err)
		}

		tenantAndUserCtx, err := makeTenantWithUserCtx(ctx, dbPool, organizationID)
		if err != nil {
			zLogger.Sugar().Error(err)
		}

		totalNotiProcessedPerTenant, err := migrateNotificationClassFilter(tenantAndUserCtx, dbPool, organizationID)
		if err != nil {
			return fmt.Errorf("migrateNotificationClassName: %s", err)
		}
		totalNotiProcessed += totalNotiProcessedPerTenant
	}
	zLogger.Sugar().Infof("There is/are %d notification migrated", totalNotiProcessed)
	zLogger.Sugar().Info("----- DONE: Migrating notification class filter job -----")
	return nil
}

func migrateNotificationClassFilter(ctx context.Context, dbPool *pgxpool.Pool, organizationID string) (int, error) {
	totalProcessedPerTenant := 0
	// only select notification that has class_ids to extract and migrate
	conditionStr := `
	AND in2.target_groups->'class_filter'->>'type' = 'NOTIFICATION_TARGET_GROUP_SELECT_LIST'
	AND jsonb_array_length(in2.target_groups -> 'class_filter'->'class_ids') > 0
	`
	if err := database.ExecInTx(ctx, dbPool, func(ctx context.Context, tx pgx.Tx) error {
		offset := 0
		notifications, err := getInfoNotificationWithOffset(ctx, tx, offset, organizationID, conditionStr)
		if err != nil {
			return fmt.Errorf("failed getInfoNotificationWithOffset: %s", err)
		}

		mapNotificationIDAndClassIDs := make(map[string][]string, 0)
		for len(notifications) > 0 {
			for _, noti := range notifications {
				targetGroup := &entities.InfoNotificationTarget{}
				err = noti.TargetGroups.AssignTo(targetGroup)
				if err != nil {
					return fmt.Errorf("failed assign target group: %v", err)
				}

				if len(targetGroup.ClassFilter.ClassIDs) > 0 {
					mapNotificationIDAndClassIDs[noti.NotificationID.String] = targetGroup.ClassFilter.ClassIDs
				}
			}

			err = BulkInsertNotificationFilter(ctx, tx, "class", organizationID, mapNotificationIDAndClassIDs)
			if err != nil {
				return fmt.Errorf("failed BulkInsertNotificationFilter: %v", err)
			}
			totalProcessedPerTenant += len(notifications)

			offset += len(notifications)
			notifications, err = getInfoNotificationWithOffset(ctx, tx, offset, organizationID, conditionStr)
			if err != nil {
				return fmt.Errorf("failed getInfoNotificationWithOffset")
			}
		}
		return nil
	}); err != nil {
		return 0, fmt.Errorf("database.ExecInTx err: %v", err)
	}

	return totalProcessedPerTenant, nil
}
