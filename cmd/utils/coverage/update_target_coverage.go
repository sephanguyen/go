...
func updateTargetCoverage(cmd *cobra.Command, args []string) error {
	if address == "" {
	...
	// call send coverage
	_, err = client.UpdateTargetCoverage(ctx, &pb.UpdateTargetCoverageRequest{
	...
	if err != nil {
		return fmt.Errorf("client.UpdateTargetCoverage: %w", err)
	}
	return nil
}
