...
func TestUpcomingLiveLessonNotificationSubscription_Subscribe(t *testing.T) {
	t.Parallel()
	...
	virtualLessonRepo := new(mock_repositories.MockVirtualLessonRepo)
	...
	handler := &consumers.UpcomingLiveLessonNotificationHandler{
	...
	subscriber := &UpcomingLiveLessonNotificationSubscription{
		Logger:            ctxzap.Extract(ctx),
		JSM:               jsm,
