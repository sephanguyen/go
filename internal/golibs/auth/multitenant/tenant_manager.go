...
type tenantManager struct {
	...
	gcpApp           *gcp.App
	gcpTenantManager internal_auth.GCPTenantManager
...
func (tm *tenantManager) TenantClient(ctx context.Context, tenantID string) (TenantClient, error) {
	...
	if !isTenantCached {
		...
		if err := internal_auth.IsScryptHashValid(config.HashConfig); err != nil {
			...
			if err := internal_auth.IsScryptHashValid(fallbackTenantConfig.HashConfig); err != nil {
...
func (tm *tenantManager) ProjectID() string {
	return tm.gcpApp.ProjectID
}
