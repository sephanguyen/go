...
func TestGeneratePublicObjectURL(t *testing.T) {
	...
	for _, tc := range testCases {
		...
		t.Run(tc.name, func(t *testing.T) {
			got := gc.GeneratePublicObjectURL(tc.name)
			if got != tc.want {
				t.Errorf("gc.GeneratePublicObjectURL(%q) = %q, expected %q", tc.name, got, tc.want)
			}
		})
	}
}
