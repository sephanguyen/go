...
func TestCreateOrderService_OrderItemUpdate(t *testing.T) {
	t.Parallel()
	...
	testcases := []utils.TestCase{
		...
		{
			Name: "Fail case: Error when mutation student product for update order",
			Ctx:  interceptors.ContextWithUserID(ctx, constant.UserID),
			Req: map[string]utils.OrderItemData{
				"key_1": utils.OrderItemData{
					Order: entities.Order{
			...
			Setup: func(ctx context.Context) {
				...
				studentProductService.On("MutationStudentProductForUpdateOrder", ctx, tx, mock.Anything, mock.Anything).Return(entities.StudentProduct{}, entities.StudentProduct{}, constant.ErrDefault)
			},
		},
		{
		...
		{
			Name: "Fail case: Error when create bill item for update order",
			Ctx:  interceptors.ContextWithUserID(ctx, constant.UserID),
			Req: map[string]utils.OrderItemData{
				"key_1": {
					Order: entities.Order{
			...
			Setup: func(ctx context.Context) {
				...
				billingService.On("CreateBillItemForOrderUpdate", mock.Anything, mock.Anything, mock.Anything).Return(constant.ErrDefault)
			},
		},
		{
			Name: "Happy case: Update order item successfully",
			Ctx:  interceptors.ContextWithUserID(ctx, constant.UserID),
			Req: map[string]utils.OrderItemData{
				"key_1": {
					Order: entities.Order{
			...
			Setup: func(ctx context.Context) {
				...
				productPriceService.On("GetProductPricesByProductIDAndPriceType", ctx, tx, mock.Anything, mock.Anything).Return([]entities.ProductPrice{{}}, nil)
				studentService.On("CheckIsEnrolledInOrgByStudentIDAndTime", mock.Anything, mock.Anything, mock.Anything, mock.Anything).Return(true, nil)
				billingService.On("CreateBillItemForOrderUpdate", ctx, tx, mock.Anything).Return(nil)
				studentPackageService.On("MutationStudentPackageForUpdateOrder", ctx, tx, mock.Anything).Return([]*npb.EventStudentPackage{{}}, nil)
			},
		},
	}
	...
	for _, testCase := range testcases {
		t.Run(testCase.Name, func(t *testing.T) {
			...
			message, elasticData, err := s.OrderItemUpdate(testCase.Ctx, tx, testCase.Req.(map[string]utils.OrderItemData))
