...
func TestCreateOrderService_OrderItemEnrollment(t *testing.T) {
	t.Parallel()
	...
	testcases := []utils.TestCase{
		{
			Name: "Fail case: Error when check enrollment status",
		...
		},
		{
			...
			Setup: func(ctx context.Context) {
				...
				productService.On("VerifiedProductWithStudentInfoReturnProductInfoAndBillingType", ctx, tx, mock.Anything).Return(entities.Product{}, true, true, pb.ProductType_PRODUCT_TYPE_MATERIAL, "grade_name", entities.ProductSetting{}, constant.ErrDefault)
			},
		},
		{
		...
		},
		{
			...
			Setup: func(ctx context.Context) {
				...
				}, true, true, pb.ProductType_PRODUCT_TYPE_MATERIAL, "grade_name", entities.ProductSetting{}, constant.ErrDefault)
			},
		},
		{
	...
	for _, testCase := range testcases {
		t.Run(testCase.Name, func(t *testing.T) {
			...
			message, elasticData, err := s.OrderItemEnrollment(testCase.Ctx, tx, testCase.Req.(map[string]utils.OrderItemData))
			if err != nil {
				fmt.Println(err)
			}
