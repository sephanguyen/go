...
func TestCreateOrderService_OrderItemCreate(t *testing.T) {
	t.Parallel()
	...
	testcases := []utils.TestCase{
				...
				"key_1": utils.OrderItemData{
					Order: entities.Order{
						...
						OrderType: pgtype.Text{
							String: pb.OrderType_ORDER_TYPE_ENROLLMENT.String(),
						},
					},
		...
		{
			Name: "Fail case: Error checking enrollment status in location",
			Ctx:  interceptors.ContextWithUserID(ctx, constant.UserID),
			Req: map[string]utils.OrderItemData{
				"key_1": utils.OrderItemData{
					Order: entities.Order{
			...
			Setup: func(ctx context.Context) {
				productService.
					On("VerifiedProductWithStudentInfoReturnProductInfoAndBillingType", ctx, tx, mock.Anything).
					Return(entities.Product{}, true, true, pb.ProductType_PRODUCT_TYPE_MATERIAL, "grade_name", entities.ProductSetting{}, nil)
				studentService.On("IsEnrolledInLocation", ctx, tx, mock.Anything).Return(false, constant.ErrDefault)
			},
		},
		{
			Name: "Fail case: Error getting product setting",
			Ctx:  interceptors.ContextWithUserID(ctx, constant.UserID),
			Req: map[string]utils.OrderItemData{
				"key_1": utils.OrderItemData{
					Order: entities.Order{
			...
			Setup: func(ctx context.Context) {
				productService.
					On("VerifiedProductWithStudentInfoReturnProductInfoAndBillingType", ctx, tx, mock.Anything).
					Return(entities.Product{}, true, true, pb.ProductType_PRODUCT_TYPE_MATERIAL, "grade_name", entities.ProductSetting{}, nil)
				studentService.On("IsEnrolledInLocation", ctx, tx, mock.Anything).Return(false, nil)
				studentProductService.On("ValidateProductSettingForCreateOrder", ctx, tx, mock.Anything).Return(constant.ErrDefault)
			},
		},
		{
			Name: "Fail case: Error when create student product",
			Ctx:  interceptors.ContextWithUserID(ctx, constant.UserID),
			Req: map[string]utils.OrderItemData{
				"key_1": utils.OrderItemData{
					Order: entities.Order{
			...
			Setup: func(ctx context.Context) {
				...
				productService.On("VerifiedProductWithStudentInfoReturnProductInfoAndBillingType", ctx, tx, mock.Anything).Return(entities.Product{}, true, true, pb.ProductType_PRODUCT_TYPE_MATERIAL, "grade_name", entities.ProductSetting{}, nil)
				studentProductService.On("CreateStudentProduct", ctx, tx, mock.Anything).Return(entities.StudentProduct{}, constant.ErrDefault)
			},
		},
		{
		...
		{
			Name: "Fail case: Error when create bill item for order create",
			Ctx:  interceptors.ContextWithUserID(ctx, constant.UserID),
			Req: map[string]utils.OrderItemData{
				"key_1": {
					Order: entities.Order{
			...
			Setup: func(ctx context.Context) {
				...
				billingService.On("CreateBillItemForOrderCreate", ctx, tx, mock.Anything).Return(constant.ErrDefault)

			},
		},
		{
			Name: "Fail case: Error when create create associated student product",
			Ctx:  interceptors.ContextWithUserID(ctx, constant.UserID),
			Req: map[string]utils.OrderItemData{
				"key_1": {
					Order: entities.Order{
	...
	for _, testCase := range testcases {
		t.Run(testCase.Name, func(t *testing.T) {
			...
			message, elasticData, err := s.OrderItemCreate(testCase.Ctx, tx, testCase.Req.(map[string]utils.OrderItemData))
			if err != nil {
				fmt.Println(err)
			}
