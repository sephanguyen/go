...
func (s *CreateOrderService) OrderItemWithdrawal(ctx context.Context, tx database.QueryExecer, mapKeyWithOrderItemData map[string]utils.OrderItemData) (
	...
	for key := range mapKeyWithOrderItemData {
		...
		err = s.StudentService.ValidateStudentStatusForOrderType(ctx, tx, pb.OrderType_ORDER_TYPE_WITHDRAWAL, orderItemData.StudentInfo, orderItemData.Order.LocationID.String, orderItemData.OrderItem.EffectiveDate.AsTime())
		if err != nil {
			return
		}
		...
		orderItemData.StudentProduct, err = s.StudentProductService.MutationStudentProductForWithdrawalOrder(ctx, tx, orderItemData)
		if err != nil {
			return
		}
		...
		err = s.BillingService.CreateBillItemForOrderWithdrawal(ctx, tx, orderItemData)
		if err != nil {
			return
		}

		err = s.StudentProductService.DeleteAssociatedStudentProductByAssociatedStudentProductID(ctx, tx, orderItemData)
		if err != nil {
			return
		}
