...
func TestCreateOrderService_OrderItemLOA(t *testing.T) {
	t.Parallel()
	...
	testcases := []utils.TestCase{
		...
		{
			Name: "Fail case: Error when verify product setting for LOA order",
			Ctx:  interceptors.ContextWithUserID(ctx, constant.UserID),
			Req: map[string]utils.OrderItemData{
				"key_1": utils.OrderItemData{
					Order: entities.Order{
			...
			Setup: func(ctx context.Context) {
				...
				studentProductService.On("ValidateProductSettingForLOAOrder", ctx, tx, mock.Anything).Return(constant.ErrDefault)
			},
		},
		{
			Name: "Fail case: Error when mutation student product for LOA order",
			Ctx:  interceptors.ContextWithUserID(ctx, constant.UserID),
			Req: map[string]utils.OrderItemData{
				"key_1": utils.OrderItemData{
					Order: entities.Order{
			...
			Setup: func(ctx context.Context) {
				...
				studentProductService.On("MutationStudentProductForLOAOrder", ctx, tx, mock.Anything, mock.Anything).Return(entities.StudentProduct{}, constant.ErrDefault)
			},
		},
		{
		...
		{
			Name: "Fail case: Error when create bill item for order LOA",
			Ctx:  interceptors.ContextWithUserID(ctx, constant.UserID),
			Req: map[string]utils.OrderItemData{
				"key_1": {
					Order: entities.Order{
			...
			Setup: func(ctx context.Context) {
				...
				billingService.On("CreateBillItemForOrderLOA", ctx, tx, mock.Anything).Return(constant.ErrDefault)
		...
		{
			Name: "Happy case: Create LOA order successfully",
			Ctx:  interceptors.ContextWithUserID(ctx, constant.UserID),
			Req: map[string]utils.OrderItemData{
				"key_1": {
					Order: entities.Order{
	...
	for _, testCase := range testcases {
		t.Run(testCase.Name, func(t *testing.T) {
			...
			message, elasticData, err := s.OrderItemLoa(testCase.Ctx, tx, testCase.Req.(map[string]utils.OrderItemData))
			if err != nil {
				fmt.Println(err)
			}
