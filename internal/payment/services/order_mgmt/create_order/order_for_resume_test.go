...
func TestCreateOrderService_OrderItemResume(t *testing.T) {
	t.Parallel()
	...
	testcases := []utils.TestCase{
		{
			Name: "Fail case: Error checking student status in location",
		...
		},
		{
			...
			Setup: func(ctx context.Context) {
				...
				studentProductService.On("MutationStudentProductForResumeOrder", ctx, tx, mock.Anything, mock.Anything).Return(entities.StudentProduct{}, constant.ErrDefault)
			},
		},
		{
			...
			Setup: func(ctx context.Context) {
				studentService.On("ValidateStudentStatusForOrderType", ctx, tx, mock.Anything, mock.Anything, mock.Anything, mock.Anything).Return(nil)
				productService.On("VerifiedProductWithStudentInfoReturnProductInfoAndBillingType", ctx, tx, mock.Anything).Return(entities.Product{}, true, true, pb.ProductType_PRODUCT_TYPE_MATERIAL, "grade_name", entities.ProductSetting{}, nil)
				studentProductService.On("MutationStudentProductForResumeOrder", ctx, tx, mock.Anything, mock.Anything).Return(entities.StudentProduct{}, nil)
		...
		},
		{
			...
			Setup: func(ctx context.Context) {
				studentService.On("ValidateStudentStatusForOrderType", ctx, tx, mock.Anything, mock.Anything, mock.Anything, mock.Anything).Return(nil)
				studentProductService.On("MutationStudentProductForResumeOrder", ctx, tx, mock.Anything, mock.Anything, mock.Anything).Return(entities.StudentProduct{}, nil)
		...
		{
			Name: "Fail case: Error when check is enrolled in order by student product",
			Ctx:  interceptors.ContextWithUserID(ctx, constant.UserID),
			Req: map[string]utils.OrderItemData{
				"key_1": {
					Order: entities.Order{
	...
	for _, testCase := range testcases {
		t.Run(testCase.Name, func(t *testing.T) {
			...
			message, elasticData, err := s.OrderItemResume(testCase.Ctx, tx, testCase.Req.(map[string]utils.OrderItemData))
			if err != nil {
				fmt.Println(err)
			}
