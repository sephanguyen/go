...
func TestCreateOrderService_OrderItemGraduate(t *testing.T) {
	t.Parallel()
	...
	testcases := []utils.TestCase{
				...
				"key_1": utils.OrderItemData{
					Order: entities.Order{
						...
						OrderType: pgtype.Text{
							String: pb.OrderType_ORDER_TYPE_GRADUATE.String(),
						},
					},
		...
		{
			Name: "Fail case: Error when mutation student product for Graduate order",
			Ctx:  interceptors.ContextWithUserID(ctx, constant.UserID),
			Req: map[string]utils.OrderItemData{
				"key_1": utils.OrderItemData{
					Order: entities.Order{
			...
			Setup: func(ctx context.Context) {
				...
				studentProductService.On("MutationStudentProductForGraduateOrder", ctx, tx, mock.Anything, mock.Anything).Return(entities.StudentProduct{}, constant.ErrDefault)
			},
		},
		{
		...
		{
			Name: "Fail case: Error when create bill item for order Graduate",
			Ctx:  interceptors.ContextWithUserID(ctx, constant.UserID),
			Req: map[string]utils.OrderItemData{
				"key_1": {
					Order: entities.Order{
	...
	for _, testCase := range testcases {
		t.Run(testCase.Name, func(t *testing.T) {
			...
			message, elasticData, err := s.OrderItemGraduate(testCase.Ctx, tx, testCase.Req.(map[string]utils.OrderItemData))
			if err != nil {
				fmt.Println(err)
			}
