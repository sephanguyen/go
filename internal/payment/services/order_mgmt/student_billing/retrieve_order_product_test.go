...
func TestRetrieveOrderProductList(t *testing.T) {
	t.Parallel()
	...
	var (
		...
		productService        *mockServices.IProductServiceForStudentBilling
	)
	...
	invaidProductType := "invalid_product_type"
	...
	var invalidMapBillItems_PackageType_UpcomingBillItem = make(map[string]*entities.BillItem, 2)
	invalidMapBillItems_PackageType_UpcomingBillItem["student_product_id_1"] = &entities.BillItem{
		StudentProductID: pgtype.Text{
			String: "student_product_id_1",
			Status: pgtype.Present,
		},
	...
	invalidMapBillItems_PackageType_UpcomingBillItem["student_product_id_2"] = &entities.BillItem{
		StudentProductID: pgtype.Text{
			String: "student_product_id_2",
			Status: pgtype.Present,
		},
	...
	TestCases := []utils.TestCase{
		...
		},
		{
			...
			Setup: func(ctx context.Context) {
				studentProductService.On("GetStudentProductByStudentIDAndLocationIDs",
					ctx,
					mock.Anything,
					mock.Anything,
					mock.Anything,
					mock.Anything,
					mock.Anything,
				).Return([]string{}, []*entities.StudentProduct{}, 0, constant.ErrDefault)
			},
		},
		{
		...
		{
			Name: "Fail case: Fail when get product setting",
			Ctx:  interceptors.ContextWithUserID(Ctx, constant.UserID),
			Req: &pb.RetrieveListOfOrderProductsRequest{
				StudentId: "1",
				Paging:    &cpb.Paging{Limit: 10, Offset: &cpb.Paging_OffsetInteger{OffsetInteger: 10}},
			...
			Setup: func(ctx context.Context) {
				...
				billItemService.On("GetMapPresentAndFutureBillItemInfo", ctx, mock.Anything, mock.Anything, mock.Anything).Return(invalidMapBillItems_PackageType_UpcomingBillItem, nil)
				...
				productService.On("GetProductSettingByProductID", ctx, mock.Anything, mock.Anything).Return(entities.ProductSetting{}, constant.ErrDefault)
			},
		},
		{
			...
			Setup: func(ctx context.Context) {
				...
				billItemService.On("GetMapPresentAndFutureBillItemInfo", ctx, mock.Anything, mock.Anything, mock.Anything).Return(invalidMapBillItems_PackageType_UpcomingBillItem, nil)
				billItemService.On("GetMapPastBillItemInfo", ctx, mock.Anything, mock.Anything, mock.Anything).Return(map[string]*entities.BillItem{}, nil)
				productService.On("GetProductSettingByProductID", ctx, mock.Anything, mock.Anything).Return(entities.ProductSetting{}, nil)
		...
		},
		{
			...
			Setup: func(ctx context.Context) {
				...
				billItemService.On("GetMapPresentAndFutureBillItemInfo", ctx, mock.Anything, mock.Anything, mock.Anything).Return(mapBillItems_PackageType, nil)
				billItemService.On("GetMapPastBillItemInfo", ctx, mock.Anything, mock.Anything, mock.Anything).Return(mapBillItems_PackageType, nil)
				productService.On("GetProductSettingByProductID", ctx, mock.Anything, mock.Anything).Return(entities.ProductSetting{}, nil)
				billItemService.On("GetUpcomingBilling", ctx, mock.Anything, mock.Anything, mock.Anything, mock.Anything).Return(&entities.BillItem{}, nil)
			},
		},
	...
	for _, testCase := range TestCases {
		t.Run(testCase.Name, func(t *testing.T) {
			...
			productService = new(mockServices.IProductServiceForStudentBilling)
			...
			resp, err := s.RetrieveListOfOrderProducts(testCase.Ctx, testCase.Req.(*pb.RetrieveListOfOrderProductsRequest))
			if err != nil {
				fmt.Println(err)
			}
			...
			if testCase.ExpectedErr != nil {
				...
				assert.Equal(t, resp.TotalItems, uint32(2))
			}

			mock.AssertExpectationsForObjects(t, db, studentProductService, billItemService, orderService, materialService, productService)
		})
	}
}
