...
func TestImportAccountingCategory(t *testing.T) {
	...
	db := new(mockDb.Ext)
	tx := new(mockDb.Tx)
	accountingCategoryRepo := new(mockRepositories.MockAccountingCategoryRepo)
	...
	testcases := []utils.TestCase{
		{
			...
			Req:         &pb.ImportAccountingCategoryRequest{},
		...
		{
			Name:        "invalid file - first column name (toLowerCase) != accounting_category_id",
			Ctx:         interceptors.ContextWithUserID(ctx, constant.UserID),
			ExpectedErr: status.Error(codes.InvalidArgument, "csv file invalid format - first column (toLowerCase) should be 'accounting_category_id'"),
			Req: &pb.ImportAccountingCategoryRequest{
		...
		{
			Name:        "invalid file - third column name (toLowerCase) != remarks",
			Ctx:         interceptors.ContextWithUserID(ctx, constant.UserID),
		...
		},
		{
			...
			ExpectedResp: &pb.ImportAccountingCategoryResponse{
				Errors: []*pb.ImportAccountingCategoryResponse_ImportAccountingCategoryError{
					{
						RowNumber: 3,
						Error:     fmt.Sprintf("unable to parse accounting category item: %s", fmt.Errorf("error parsing is_archived")),
					},
					{
						RowNumber: 4,
						Error:     fmt.Sprintf("unable to parse accounting category item: %s", fmt.Errorf("missing mandatory data: is_archived")),
					},
					{
						RowNumber: 6,
						Error:     fmt.Sprintf("unable to create new accounting category item: %s", pgx.ErrTxClosed),
					},
					{
						RowNumber: 7,
						Error:     fmt.Sprintf("unable to update accounting category item: %s", pgx.ErrTxClosed),
					},
				},
			},
			Setup: func(ctx context.Context) {
				accountingCategoryRepo.On("Create", ctx, tx, mock.Anything).Once().Return(nil)
				accountingCategoryRepo.On("Update", ctx, tx, mock.Anything).Once().Return(nil)
				accountingCategoryRepo.On("Create", ctx, tx, mock.Anything).Once().Return(pgx.ErrTxClosed)
				accountingCategoryRepo.On("Update", ctx, tx, mock.Anything).Once().Return(pgx.ErrTxClosed)
	...
	for _, testCase := range testcases {
		t.Run(testCase.Name, func(t *testing.T) {
			...
			resp, err := s.ImportAccountingCategory(testCase.Ctx, testCase.Req.(*pb.ImportAccountingCategoryRequest))
			if err != nil {
				//fmt.Println(err)
			}
			...
			if testCase.ExpectedErr != nil {
				...
				expectedResp := testCase.ExpectedResp.(*pb.ImportAccountingCategoryResponse)
			...
			mock.AssertExpectationsForObjects(t, db, accountingCategoryRepo)
		})
	}
}
