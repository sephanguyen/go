...
func TestImportProduct_Grade(t *testing.T) {
	t.Parallel()
	ctx, cancel := context.WithTimeout(context.Background(), utils.TimeOut)
	defer cancel()
	var (
		mockProductGradeRepo *mockRepositories.MockProductGradeRepo
		db                   *mockDb.Ext
		mockTx               *mockDb.Tx
	)
	...
	testcases := []utils.TestCase{
		...
		{
			Name:        "invalid file - second column name (toLowerCase) != grade_id",
			Ctx:         interceptors.ContextWithUserID(ctx, constant.UserID),
			ExpectedErr: status.Error(codes.InvalidArgument, "csv file invalid format - second column (toLowerCase) should be 'grade_id'"),
			Req: &pb.ImportProductAssociatedDataRequest{
				ProductAssociatedDataType: pb.ProductAssociatedDataType_PRODUCT_ASSOCIATED_DATA_TYPE_GRADE,
		...
		{
			Name: "parsing product grade with error lines in response",
			Ctx:  interceptors.ContextWithUserID(ctx, constant.UserID),
			Req: &pb.ImportProductAssociatedDataRequest{
				ProductAssociatedDataType: pb.ProductAssociatedDataType_PRODUCT_ASSOCIATED_DATA_TYPE_GRADE,
				Payload: []byte(`product_id,grade_id
			...
			ExpectedResp: &pb.ImportProductAssociatedDataResponse{
				Errors: []*pb.ImportProductAssociatedDataResponse_ImportProductAssociatedDataError{
					{
						RowNumber: 3,
						Error:     fmt.Sprintf("unable to parse product associated data item: %s", fmt.Errorf("missing mandatory data: grade_id")),
					},
					{
						RowNumber: 4,
						Error:     fmt.Sprintf("unable to parse product associated data item: %s", fmt.Errorf("missing mandatory data: product_id")),
					},
				},
			},
			Setup: func(ctx context.Context) {
				...
				mockProductGradeRepo.On("Upsert", ctx, mockTx, mock.Anything, mock.Anything).Return(nil)
			},
		},
		{
		...
		},
		{
			...
			ExpectedResp: &pb.ImportProductAssociatedDataResponse{
				Errors: []*pb.ImportProductAssociatedDataResponse_ImportProductAssociatedDataError{
					{
						RowNumber: 2,
						Error:     fmt.Sprintf("unable to create new product grade item: error something"),
					},
				},
			},
			Setup: func(ctx context.Context) {
				mockTx.On("Rollback", mock.Anything).Return(nil)
				db.On("Begin", mock.Anything).Return(mockTx, nil)
				mockProductGradeRepo.On("Upsert", ctx, mockTx, mock.Anything, mock.Anything).Once().Return(fmt.Errorf("error something"))
			},
		},
	}
	...
	for _, testCase := range testcases {
		t.Run(testCase.Name, func(t *testing.T) {
			...
			mockProductGradeRepo = new(mockRepositories.MockProductGradeRepo)
			testCase.Setup(testCase.Ctx)
			s := &ImportMasterDataService{
				DB:               db,
				ProductGradeRepo: mockProductGradeRepo,
			}
			...
			mock.AssertExpectationsForObjects(t, db, mockProductGradeRepo)
		})
	}
}
