...
func PackageCourseFeeRepoWithSqlMock() (*PackageCourseFeeRepo, *testutil.MockDB, *mock_database.Tx) {
	packageCourseFeeRepo := &PackageCourseFeeRepo{}
	return packageCourseFeeRepo, testutil.NewMockDB(), &mock_database.Tx{}
}

func TestPackageCourseFee_Upsert(t *testing.T) {
	t.Parallel()
	mockDb := &mock_db.Ext{}
	packageCourseFeeRepo := &PackageCourseFeeRepo{}
			...
			Req: []*entities.PackageCourseFee{
				{
					...
					FeeID:     pgtype.Text{String: "3", Status: pgtype.Present},
				},
			},
			ExpectedErr: nil,
		...
		},
		{
			...
			Req: []*entities.PackageCourseFee{
				...
				},
				{
					...
					FeeID:     pgtype.Text{String: "4", Status: pgtype.Present},
				},
			},
			ExpectedErr: nil,
			Setup: func(ctx context.Context) {
	...
	for _, testCase := range testCases {
		...
		testCase.Setup(ctx)
		err := packageCourseFeeRepo.Upsert(ctx, mockDb, pgtype.Text{String: "1", Status: pgtype.Present}, testCase.Req.([]*entities.PackageCourseFee))
...
func TestPackageCourseFee_GetToTalAssociatedByCourseIDAndPackageID(t *testing.T) {
	t.Parallel()
