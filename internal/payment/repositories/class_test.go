...
func TestClassRepo_GetMapClassWithLocationByClassIDs(t *testing.T) {
	t.Parallel()
	ctx, cancel := context.WithTimeout(context.Background(), time.Second)
	defer cancel()

	var (
		classRepoWithSqlMock *ClassRepo
	...
	for _, testCase := range testcases {
		t.Run(testCase.Name, func(t *testing.T) {
			classRepoWithSqlMock, mockDB = ClassRepoWithSqlMock()
			testCase.Setup(testCase.Ctx)

			_, err := classRepoWithSqlMock.GetMapClassWithLocationByClassIDs(testCase.Ctx, mockDB.DB, []string{"1", "2"})
