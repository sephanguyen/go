...
func TestLessonEventSubscription_Subscribe(t *testing.T) {
	t.Parallel()
	...
	tests := []struct {
	...
	}{
		{
			...
			setup: func(ctx context.Context) {
				...
				mockJsm.On("QueueSubscribe", constants.SubjectLesson, constants.QueueTimesheetLesson, mock.Anything, mock.Anything).Once().Return(nil, nil)
			},
		},
		{
