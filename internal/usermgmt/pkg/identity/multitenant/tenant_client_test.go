...
func aMockTenantClient() *tenantClient {
	...
	tc.tenantID = "example-id"
	tc.HashConfig = aValidScryptHash()
...
func TestTenantClient_GetMethods(t *testing.T) {
	t.Parallel()

	tenantClient := aMockTenantClient()

	assert.Equal(t, tenantClient.tenantID, tenantClient.TenantID())
	assert.Equal(t, tenantClient.HashConfig, tenantClient.GetHashConfig())
	assert.NotNil(t, tenantClient.gcpClient)
	assert.NotNil(t, tenantClient.gcpUtils)
}
...
func TestTenantClient_GetUser(t *testing.T) {
	...
	testCases := []struct {
		...
		},
		{
			...
			expectedUser: NewUserFromGCPUserRecord(validUserRecord),
			expectedErr:  nil,
			setupFunc: func(ctx context.Context) {
				tc = aMockTenantClient()
...
/*func TestTenantClient_UpdateUser(t *testing.T) {
	...
	testCases := []struct {
		...
		},
		{
			...
			inputUser:      anUserWithValidInfo(),
			expectedResult: NewUserFromGCPUserRecord(userRecord),
			expectedErr:    nil,
			setupFunc: func(ctx context.Context) {
				tc = aMockTenantClient()
...
func TestTenantClient_ImportUsers(t *testing.T) {
	...
	testCases := []struct {
		...
		},
		{
			...
			inputUsers:     Users{anUserWithValidInfo()},
			expectedResult: NewUserFromGCPUserRecord(userRecord),
			expectedErr:    nil,
			setupFunc: func(ctx context.Context) {
				tc = aMockTenantClient()
