...
func userAddressRepoWithSqlMock() (*UserAddressRepo, *testutil.MockDB) {
	repo := &UserAddressRepo{}
	return repo, testutil.NewMockDB()
}

func TestUserAddressRepo_SoftDeleteByUsers(t *testing.T) {
	t.Parallel()
	ctx, cancel := context.WithTimeout(context.Background(), time.Second)
	defer cancel()

	r, mockDB := userAddressRepoWithSqlMock()
...
func TestUserAddressRepo_Upsert(t *testing.T) {
	t.Parallel()
	...
	testCases := []TestCase{
		{
			name: "happy case",
			req: []*entity.UserAddress{
				{
					UserAddressID: pgtype.Text{String: uuid.NewString(), Status: pgtype.Present},
					UserID:        pgtype.Text{String: uuid.NewString(), Status: pgtype.Present},
					AddressType:   pgtype.Text{String: uuid.NewString(), Status: pgtype.Present},
	...
	for _, testCase := range testCases {
		...
		testCase.setup(ctx)
		err := userAddressRepo.Upsert(ctx, db, testCase.req.([]*entity.UserAddress))
...
func TestUserAddressRepo_GetByUserID(t *testing.T) {
	t.Parallel()
	ctx, cancel := context.WithTimeout(context.Background(), time.Second)
	defer cancel()
	...
	_, userAddressValues := (&entity.UserAddress{}).FieldMap()
