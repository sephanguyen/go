...
func TestStaffService_UserToSchoolAdmin(t *testing.T) {
	t.Parallel()
	...
	for _, testCase := range testCases {
		school_admin := userToSchoolAdmin(
			testCase.req.(params).user,
			int32(testCase.req.(params).schoolIDs[0]),
		)

		if testCase.expectedErr != nil {
			assert.NotEmpty(t, school_admin)
		}
	}
}

func TestStaffService_CreateSchoolAdmin(t *testing.T) {
	t.Parallel()
	ctx, cancel := context.WithTimeout(context.Background(), 15*time.Second)
	defer cancel()
	...
	testCases := []TestCase{
		{
			...
			setup: func(ctx context.Context) {
				schoolAdminRepo.On("CreateMultiple", ctx, tx, mock.Anything).Once().Return(nil)
			},
		},
	}
	...
	for _, testCase := range testCases {
		...
		err := s.createSchoolAdmin(ctx, tx, testCase.req.(params).user, testCase.req.(params).schoolIDs)
