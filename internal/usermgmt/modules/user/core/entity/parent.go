...
func (parents Parents) Emails() (emails []string) {
	for _, parent := range parents {
		if parent.LegacyUser.Email.Status == pgtype.Present {
			emails = append(emails, parent.LegacyUser.Email.String)
		}
	}
	return
}

func (parents Parents) PhoneNumbers() (phoneNumbers []string) {
	for _, parent := range parents {
...
func (parents Parents) Users() LegacyUsers {
	users := make(LegacyUsers, 0, len(parents))
	for _, parent := range parents {
