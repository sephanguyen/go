...
func TestReallocateStudentEnrollmentStatusSubscriber_Subscribe(t *testing.T) {
	t.Parallel()
	_, cancel := context.WithTimeout(context.Background(), time.Second*10)
	defer cancel()

	jsm := new(mock_nats.JetStreamManagement)
	subscriber := &ReallocateStudentEnrollmentStatusSubscriber{
		JSM:                        jsm,
	...
	testCases := []struct {
	...
	}{
		{
			name: "success",
			setup: func(ctx context.Context) {
				jsm.On("QueueSubscribe", mock.Anything, constants.QueueEnrollmentStatusAssignment, mock.Anything, mock.Anything).Once().Return(nil, nil)
			},
			expectedErr: nil,
		},
		{
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			ctxWithResourcePath := golibs.ResourcePathToCtx(context.Background(), fmt.Sprint(constants.ManabieSchool))
