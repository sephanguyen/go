...
func CreateSchoolConfig_InvalidArgument(t *testing.T) {
	schoolService := &SchoolService{}
	type testInput struct {
		request        *pb.CreateSchoolConfigRequest
	...
	testCases := map[string]testInput{
		"school not found": {
			...
			expectedErrMsg: "rpc error: code = InvalidArgument desc = school not found",
		},
		"missing school id": {
			request: &pb.CreateSchoolConfigRequest{
				SchoolConfig: &pb.SchoolConfig{
	...
	for caseName, testCase := range testCases {
		t.Run(caseName, func(t *testing.T) {
			resp, err := schoolService.CreateSchoolConfig(context.Background(), testCase.request)
