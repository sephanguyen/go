...
func TestCourseAccessPathRepo_Upsert(t *testing.T) {
	...
	t.Run("successfully", func(t *testing.T) {
		course1 := &domain.CourseAccessPath{
			LocationID: "location-1",
			CourseID:   "course-1",
			CreatedAt:  time.Now(),
			UpdatedAt:  time.Now(),
		}
		course2 := &domain.CourseAccessPath{
			LocationID: "location-2",
			CourseID:   "course-2",
			CreatedAt:  time.Now(),
			UpdatedAt:  time.Now(),
		}
		caps := []*domain.CourseAccessPath{course1, course2}
...
func TestCourseAccessPathRepo_Delete(t *testing.T) {
	...
	courseIDs := []string{"courseID-1", "courseID-2"}
...
func TestCourseAccessPathRepo_GetByCourseIDs(t *testing.T) {
	t.Parallel()
	ctx, cancel := context.WithTimeout(context.Background(), time.Second)
	defer cancel()
	...
	t.Run("err select", func(t *testing.T) {
		...
		courseAccessPaths, err := r.GetByCourseIDs(ctx, mockDB.DB, ids)
		assert.True(t, errors.Is(err, puddle.ErrClosedPool))
		assert.Nil(t, courseAccessPaths)
	})
	...
	t.Run("success with select", func(t *testing.T) {
		...
		_, err := r.GetByCourseIDs(ctx, mockDB.DB, ids)
		assert.Nil(t, err)
