...
func TestTimeSlotService_ImportTimeSlots(t *testing.T) {
	ctx, cancel := context.WithTimeout(context.Background(), time.Second)
	defer cancel()
	masterDB := &mock_database.Ext{}
	bobDB := &mock_database.Ext{}
	tx := &mock_database.Tx{}
	mockTimeSlotRepo := new(mock_time_slot_repo.MockTimeSlotRepo)
	locationRepo := new(mock_location_repo.MockLocationRepo)

	TimeSlotService := NewTimeSlotService(masterDB, bobDB, mockTimeSlotRepo, locationRepo)
	...
	testCases := []struct {
		name             string
		req              *mpb.ImportTimeSlotRequest
		expectedResp     *mpb.ImportTimeSlotResponse
		expectedErr      error
	...
	}{
		{
			name: "success",
			req: &mpb.ImportTimeSlotRequest{
				Payload: []byte(`time_slot_internal_id,start_time,end_time
				1,09:00,18:00
				2,10:00,17:00`),
				LocationId: reqLocationID,
			},
			setup: func(ctx context.Context) {
				...
				mockTimeSlotRepo.On("Upsert", ctx, tx, mock.Anything, mockLocationIDs).Return(nil)
			},
			expectedResp: &mpb.ImportTimeSlotResponse{Errors: []*mpb.ImportTimeSlotResponse_ImportTimeSlotError{}},
		},
		{
			name: "error when have invalid header",
			req: &mpb.ImportTimeSlotRequest{
				Payload: []byte(`time_slot_internal_id,start_time,invalid_header
				1,09:00,18:00
				2,10:00,17:00`),
				LocationId: reqLocationID,
			},
			...
			expectedResp: nil,
			expectedErr:  status.Error(codes.InvalidArgument, "csv has invalid format, column number 3 should be end_time, got invalid_header"),
		},
		{
			name: "error when miss header",
			req: &mpb.ImportTimeSlotRequest{
				Payload: []byte(`time_slot_internal_id,start_time
				1,09:00,18:00
				2,10:00,17:00`),
				LocationId: reqLocationID,
			},
		...
		},
		{
			...
			req: &mpb.ImportTimeSlotRequest{
				Payload: []byte(`time_slot_internal_id,start_time,end_time,redundant_header
				1,09:00,18:00
				2,10:00,17:00`),
				LocationId: reqLocationID,
			},
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			tc.setup(ctx)
			resp, err := TimeSlotService.ImportTimeSlots(ctx, tc.req)
			...
			mock.AssertExpectationsForObjects(t, masterDB, mockTimeSlotRepo)
		})
	}
}
