...
func (g *CScsvPaymentRequestGenerator) GetByteContent(ctx context.Context, dataMapList []*dataMap) ([]byte, error) {
	csvData, err := GenCSVData(g.partnerCS, dataMapList, g.prefectureCodeWithNameMapped, g.UseKECFeedbackPh1)
	if err != nil {
		return nil, err
	}
...
func GenCSVData(partnerCS *entities.PartnerConvenienceStore, filePaymentDataList []*dataMap, prefectureCodeWithNameMapped map[string]string, useKECFeedbackPh1 bool) ([][]string, error) {
	...
	for _, filePaymentData := range filePaymentDataList {
		...
		invoiceRecordSlice, err := createFileInvoiceRecords(filePaymentData, createdDateStr, prefectureCodeWithNameMapped, useKECFeedbackPh1)
		if err != nil {
			return nil, err
		}
