...
type invoiceAndPaymentStatusAndCountDetail struct {
...
func (t *BasePaymentFileValidator) validateAndGetEntitiesToUpdateAndCreate(
	...
	for i, paymentNo := range paymentMaps.PaymentNumbers {
		...
		// initialize action detail variable
		var actionDetails *utils.InvoiceActionLogDetails

		actionDetails, statusCount = t.determineActionAndStatusCount(&invoiceAndPaymentStatusAndCountDetail{StatusCount: statusCount, Invoice: invoice, Payment: payment, PreviousPaymentStatus: previousPaymentStatus, PreviousInvoiceStatus: previousInvoiceStatus, BulkPaymentValidationsID: param.BulkPaymentValidationID}, featureFlags)
...
func (t *BasePaymentFileValidator) determineActionAndStatusCount(invoiceAndPaymentStatusAndCountDetail *invoiceAndPaymentStatusAndCountDetail, featureFlags *featureFlags) (*utils.InvoiceActionLogDetails, *paymentStatusCount) {
	// generate action log details based on validation entity status
	actionDetails := &utils.InvoiceActionLogDetails{
		InvoiceID:                invoiceAndPaymentStatusAndCountDetail.Invoice.InvoiceID.String,
		PaymentSequenceNumber:    invoiceAndPaymentStatusAndCountDetail.Payment.PaymentSequenceNumber.Int,
		Action:                   invoice_pb.InvoiceAction_NO_ACTION, // Ensures no action log will be created if not modified
		BulkPaymentValidationsID: invoiceAndPaymentStatusAndCountDetail.BulkPaymentValidationsID,
	}

	statusCount := invoiceAndPaymentStatusAndCountDetail.StatusCount

	switch invoiceAndPaymentStatusAndCountDetail.Invoice.Status.String {
	...
	case invoice_pb.InvoiceStatus_ISSUED.String():
		if featureFlags.UseBulkAddValidatePh2 && invoiceAndPaymentStatusAndCountDetail.Payment.PaymentStatus.String == invoice_pb.PaymentStatus_PAYMENT_FAILED.String() {
	...
	if invoiceAndPaymentStatusAndCountDetail.PreviousInvoiceStatus == invoiceAndPaymentStatusAndCountDetail.Invoice.Status.String && invoiceAndPaymentStatusAndCountDetail.PreviousPaymentStatus == invoiceAndPaymentStatusAndCountDetail.Payment.PaymentStatus.String {
		actionDetails.Action = invoice_pb.InvoiceAction_PAYMENT_UPDATED
	}

	if featureFlags.UseBulkAddValidatePh2 {
		if invoiceAndPaymentStatusAndCountDetail.PreviousPaymentStatus == invoiceAndPaymentStatusAndCountDetail.Payment.PaymentStatus.String {
			actionDetails.Action = invoice_pb.InvoiceAction_PAYMENT_UPDATED
		}
		if invoiceAndPaymentStatusAndCountDetail.PreviousPaymentStatus != invoiceAndPaymentStatusAndCountDetail.Payment.PaymentStatus.String && invoiceAndPaymentStatusAndCountDetail.Payment.PaymentStatus.String == invoice_pb.PaymentStatus_PAYMENT_FAILED.String() {
			actionDetails.Action = invoice_pb.InvoiceAction_PAYMENT_VALIDATE_FAILED
		}

		// payment status is success after validating
		if invoiceAndPaymentStatusAndCountDetail.Payment.PaymentStatus.String == invoice_pb.PaymentStatus_PAYMENT_SUCCESSFUL.String() {
			actionDetails.Action = invoice_pb.InvoiceAction_PAYMENT_VALIDATE_SUCCESS
		}
	}

	return actionDetails, statusCount
}
