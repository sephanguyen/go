...
func TestInvoiceModifierService_CreatePaymentRequest_CS(t *testing.T) {
	t.Parallel()
	...
	testcases := []TestCase{
		{
			...
			setup: func(ctx context.Context) {
				...
				mockUnleashClient.On("IsFeatureEnabled", constant.EnableBillingMessageInCSVMessages, mock.Anything).Once().Return(false, nil)
				mockUnleashClient.On("IsFeatureEnabled", constant.EnableFormatPaymentRequestFileFields, mock.Anything).Once().Return(false, nil)
				mockUnleashClient.On("IsFeatureEnabled", constant.EnableEncodePaymentRequestFiles, mock.Anything).Once().Return(false, nil)
...
func TestInvoiceModifierService_CreatePaymentRequest_DD(t *testing.T) {
	t.Parallel()
	ctx, cancel := context.WithTimeout(context.Background(), 15*time.Second)
	defer cancel()
...
func TestInvoiceModifierService_CreatePaymentRequest_CS_EnableGcloud(t *testing.T) {
	t.Parallel()
	ctx, cancel := context.WithTimeout(context.Background(), 15*time.Second)
	defer cancel()
...
func TestInvoiceModifierService_CreatePaymentRequest_DD_EnableGcloud(t *testing.T) {
	t.Parallel()
	ctx, cancel := context.WithTimeout(context.Background(), 15*time.Second)
	defer cancel()
	...
	testcases := []TestCase{
		{
			...
			setup: func(ctx context.Context) {
				...
				mockBulkPaymentRequestRepo.On("Create", ctx, mockDB, mock.Anything).Once().Return(mockBulkPaymentRequestID, nil)
				mockUnleashClient.On("IsFeatureEnabled", constant.EnableFormatPaymentRequestFileFields, mock.Anything).Once().Return(true, nil)
				mockUnleashClient.On("IsFeatureEnabled", constant.EnableEncodePaymentRequestFiles, mock.Anything).Once().Return(true, nil)
...
func TestInvoiceModifierService_CreatePaymentRequest_CS_OptionalDueDates(t *testing.T) {
	t.Parallel()
	ctx, cancel := context.WithTimeout(context.Background(), 15*time.Second)
	defer cancel()
...
func TestInvoiceModifierService_CreatePaymentRequest_CS_EnableCSVBillingMessage(t *testing.T) {
	t.Parallel()
	ctx, cancel := context.WithTimeout(context.Background(), 15*time.Second)
	defer cancel()
	...
	testcases := []TestCase{
		{
			name: "happy case - Convenience Store CSV",
			ctx:  ctx,
			req: &invoice_pb.CreatePaymentRequestRequest{
				PaymentMethod: invoice_pb.PaymentMethod_CONVENIENCE_STORE,
				ConvenienceStoreDates: &invoice_pb.CreatePaymentRequestRequest_ConvenieceStoreDates{
			...
			setup: func(ctx context.Context) {
				...
				invoiceIDs := make([]string, len(mockPaymentInvoice))
				for i, e := range mockPaymentInvoice {
					invoiceIDs[i] = e.Invoice.InvoiceID.String
				}
				...
				mockUnleashClient.On("IsFeatureEnabled", constant.EnableGCloudUploadFeatureFlag, mock.Anything).Once().Return(true, nil)
				mockUnleashClient.On("IsFeatureEnabled", constant.EnableOptionalValidationInPaymentRequest, mock.Anything).Once().Return(true, nil)
				mockUnleashClient.On("IsFeatureEnabled", constant.EnableBillingMessageInCSVMessages, mock.Anything).Once().Return(true, nil)
				mockUnleashClient.On("IsFeatureEnabled", constant.EnableFormatPaymentRequestFileFields, mock.Anything).Once().Return(true, nil)
				mockUnleashClient.On("IsFeatureEnabled", constant.EnableEncodePaymentRequestFiles, mock.Anything).Once().Return(true, nil)
				...
				mockPaymentRepo.On("FindPaymentInvoiceByIDs", ctx, mockDB, mock.Anything).Once().Return(mockPaymentInvoice, nil)
				mockStudentPaymentDetailRepo.On("FindStudentBillingByStudentIDs", ctx, mockDB, mock.Anything).Once().Return(mockStudentBilling, nil)
				mockBillItemRepo.On("FindInvoiceBillItemMapByInvoiceIDs", ctx, mockDB, mock.Anything).Once().Return(genMockInvoiceBillItemMap(invoiceIDs), nil)
				mockInvoiceAdjustmentRepo.On("FindByInvoiceIDs", ctx, mockDB, mock.Anything).Once().Return(genMockInvoiceAdjustments(invoiceIDs), nil)
		...
		{
			name: "happy case - Convenience Store CSV invoice contains greater 6 bill item and adjustment",
			ctx:  ctx,
			req: &invoice_pb.CreatePaymentRequestRequest{
				PaymentMethod: invoice_pb.PaymentMethod_CONVENIENCE_STORE,
				ConvenienceStoreDates: &invoice_pb.CreatePaymentRequestRequest_ConvenieceStoreDates{
			...
			setup: func(ctx context.Context) {
				...
				billItems := []*entities.InvoiceBillItemMap{}
				for i := 0; i < 6; i++ {
					billItems = append(billItems, genMockInvoiceBillItemMap(invoiceIDs)...)
				}
				...
				mockPaymentRepo.On("FindPaymentInvoiceByIDs", ctx, mockDB, mock.Anything).Once().Return(mockPaymentInvoice, nil)
				mockStudentPaymentDetailRepo.On("FindStudentBillingByStudentIDs", ctx, mockDB, mock.Anything).Once().Return(mockStudentBilling, nil)
				mockBillItemRepo.On("FindInvoiceBillItemMapByInvoiceIDs", ctx, mockDB, mock.Anything).Once().Return(billItems, nil)
				mockInvoiceAdjustmentRepo.On("FindByInvoiceIDs", ctx, mockDB, mock.Anything).Once().Return(genMockInvoiceAdjustments(invoiceIDs), nil)
		...
		{
			name: "happy case - FindInvoiceBillItemMapByInvoiceIDs error",
			ctx:  ctx,
			req: &invoice_pb.CreatePaymentRequestRequest{
				PaymentMethod: invoice_pb.PaymentMethod_CONVENIENCE_STORE,
				ConvenienceStoreDates: &invoice_pb.CreatePaymentRequestRequest_ConvenieceStoreDates{
			...
			expectedErr: status.Error(codes.Internal, fmt.Sprintf("g.BillItemRepo.FindInvoiceBillItemMapByInvoiceIDs err: %v", testError)),
			setup: func(ctx context.Context) {
				mockPaymentInvoice := genMockPaymentInvoice(
					mockPaymentIDs,
					invoice_pb.PaymentStatus_PAYMENT_PENDING.String(),
				...
				mockPaymentRepo.On("FindPaymentInvoiceByIDs", ctx, mockDB, mock.Anything).Once().Return(mockPaymentInvoice, nil)
				mockStudentPaymentDetailRepo.On("FindStudentBillingByStudentIDs", ctx, mockDB, mock.Anything).Once().Return(mockStudentBilling, nil)
				mockBillItemRepo.On("FindInvoiceBillItemMapByInvoiceIDs", ctx, mockDB, mock.Anything).Once().Return(nil, testError)
		...
		},
		{
			...
			expectedErr: status.Error(codes.Internal, fmt.Sprintf("g.InvoiceAdjustmentRepo.FindByInvoiceIDs err: %v", testError)),
			setup: func(ctx context.Context) {
				mockPaymentInvoice := genMockPaymentInvoice(
					mockPaymentIDs,
					invoice_pb.PaymentStatus_PAYMENT_PENDING.String(),
				...
				mockStudentPaymentDetailRepo.On("FindStudentBillingByStudentIDs", ctx, mockDB, mock.Anything).Once().Return(mockStudentBilling, nil)
				mockBillItemRepo.On("FindInvoiceBillItemMapByInvoiceIDs", ctx, mockDB, mock.Anything).Once().Return(genMockInvoiceBillItemMap(invoiceIDs), nil)
				mockInvoiceAdjustmentRepo.On("FindByInvoiceIDs", ctx, mockDB, mock.Anything).Once().Return(nil, testError)
...
func genMockInvoiceBillItemMap(invoiceIDs []string) []*entities.InvoiceBillItemMap {
	mockData := make([]*entities.InvoiceBillItemMap, len(invoiceIDs))

	for i, invoiceID := range invoiceIDs {
		mockData[i] = &entities.InvoiceBillItemMap{
			InvoiceID:  database.Text(invoiceID),
			FinalPrice: database.Numeric(100),
		}

		billingItemDescription := orderEntities.BillingItemDescription{
			ProductName: fmt.Sprintf("test-product-%d", i),
		}

		mockData[i].BillingItemDescription.Set(billingItemDescription)
	}

	return mockData
}

func genMockInvoiceAdjustments(invoiceIDs []string) []*entities.InvoiceAdjustment {
	mockData := make([]*entities.InvoiceAdjustment, len(invoiceIDs))

	for i, invoiceID := range invoiceIDs {
		mockData[i] = &entities.InvoiceAdjustment{
			InvoiceID:   database.Text(invoiceID),
			Description: database.Text(fmt.Sprintf("desc-%d", i)),
			Amount:      database.Numeric(100),
		}
