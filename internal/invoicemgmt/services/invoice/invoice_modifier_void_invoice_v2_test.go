...
func TestInvoiceModifierService_VoidInvoiceV2(t *testing.T) {
	t.Parallel()
	...
	payment := &entities.Payment{
		...
		Amount:            database.Numeric(2000),
	}

	paymentInBulk := &entities.Payment{
		PaymentID:         database.Text("1"),
		CreatedAt:         pgtype.Timestamptz{Time: time.Now().Add(-1 * time.Hour)},
		InvoiceID:         invoiceIssued.InvoiceID,
		PaymentDate:       pgtype.Timestamptz{Time: time.Now().Add(-1 * time.Hour)},
		...
		PaymentStatus:     database.Text(invoice_pb.PaymentStatus_PAYMENT_PENDING.String()),
		Amount:            database.Numeric(2000),
		BulkPaymentID:     database.Text("test-bulk-payment"),
	}
	...
	voidInvoiceReq := &invoice_pb.VoidInvoiceRequestV2{
		InvoiceId: "1",
		Remarks:   "any",
	}
	...
	testcases := []TestCase{
		{
			name:         "happy case voiding invoice with single invoice bill item",
		...
		{
			name:         "happy case voiding invoice with multiple invoice bill items",
			ctx:          interceptors.ContextWithUserID(ctx, ctxUserID),
			req:          voidInvoiceReq,
			expectedResp: successfulResp,
			expectedErr:  nil,
		...
		{
			name:         "happy case voiding invoice without payment",
			ctx:          interceptors.ContextWithUserID(ctx, ctxUserID),
			req:          voidInvoiceReq,
			expectedResp: successfulResp,
			expectedErr:  nil,
			setup: func(ctx context.Context) {
				mockInvoiceRepo.On("RetrieveInvoiceByInvoiceID", ctx, mockDB, mock.Anything).Once().Return(invoiceIssued, nil)
				mockDB.On("Begin", ctx).Once().Return(mockTx, nil)
				mockPaymentRepo.On("GetLatestPaymentDueDateByInvoiceID", ctx, mockTx, mock.Anything).Once().Return(nil, nil)
				mockInvoiceRepo.On("Update", ctx, mockTx, mock.Anything).Once().Return(nil)
				mockInvoiceActionLogRepo.On("Create", ctx, mockTx, mock.Anything).Once().Return(nil)
				mockInvoiceBillItemRepo.On("FindAllByInvoiceID", ctx, mockTx, mock.Anything).Once().Return(&invoiceBillItemsSingle, nil)
				mockBillItemRepo.On("FindByID", ctx, mockTx, mock.Anything).Once().Return(billItem, nil)
		...
		{
			name:         "happy case voiding invoice and cancelling bulk payment",
			ctx:          interceptors.ContextWithUserID(ctx, ctxUserID),
			req:          voidInvoiceReq,
			expectedResp: successfulResp,
			expectedErr:  nil,
			setup: func(ctx context.Context) {
				mockInvoiceRepo.On("RetrieveInvoiceByInvoiceID", ctx, mockDB, mock.Anything).Once().Return(invoiceIssued, nil)
				mockDB.On("Begin", ctx).Once().Return(mockTx, nil)
				mockPaymentRepo.On("GetLatestPaymentDueDateByInvoiceID", ctx, mockTx, mock.Anything).Once().Return(paymentInBulk, nil)
		...
		{
			name:        "failed case voiding invoice without invoice bill item",
			ctx:         interceptors.ContextWithUserID(ctx, ctxUserID),
			req:         voidInvoiceReq,
			expectedErr: status.Error(codes.Internal, "error no Invoice Bill Item records found"),
			setup: func(ctx context.Context) {
				mockInvoiceRepo.On("RetrieveInvoiceByInvoiceID", ctx, mockDB, mock.Anything).Once().Return(invoiceIssued, nil)
				mockDB.On("Begin", ctx).Once().Return(mockTx, nil)
				mockPaymentRepo.On("GetLatestPaymentDueDateByInvoiceID", ctx, mockTx, mock.Anything).Once().Return(payment, nil)
		...
		{
			name: "failed no invoice id provided",
			ctx:  interceptors.ContextWithUserID(ctx, ctxUserID),
		...
		{
			name:         "failed retrieving invoice by invoice id",
			ctx:          interceptors.ContextWithUserID(ctx, ctxUserID),
			req:          voidInvoiceReq,
			expectedResp: failedResp,
			expectedErr:  status.Error(codes.Internal, "error Invoice RetrieveInvoiceByInvoiceID: test error"),
			setup: func(ctx context.Context) {
				mockInvoiceRepo.On("RetrieveInvoiceByInvoiceID", ctx, mockDB, mock.Anything).Once().Return(nil, testErr)
			},
		},
		{
			name:         "failed test -  Invalid invoice status: VOID",
			ctx:          interceptors.ContextWithUserID(ctx, ctxUserID),
			req:          voidInvoiceReq,
			expectedResp: failedResp,
		...
		{
			name:         "failed test -  Invalid invoice status: PAID",
			ctx:          interceptors.ContextWithUserID(ctx, ctxUserID),
			req:          voidInvoiceReq,
			expectedResp: failedResp,
			expectedErr:  status.Error(codes.InvalidArgument, "Invoice should be in DRAFT, ISSUED, or FAILED status"),
		...
		{
			name:         "failed test - PaymentRepo.GetLatestPaymentDueDateByInvoiceID DB Error",
			ctx:          interceptors.ContextWithUserID(ctx, ctxUserID),
			req:          voidInvoiceReq,
			expectedResp: failedResp,
		...
		{
			name:         "failed test - PaymentRepo.Update DB Error",
			ctx:          interceptors.ContextWithUserID(ctx, ctxUserID),
			req:          voidInvoiceReq,
			expectedResp: failedResp,
			expectedErr:  status.Error(codes.Internal, "error Payment Update: error Payment UpdateWithFields: tx is closed"),
			setup: func(ctx context.Context) {
				mockInvoiceRepo.On("RetrieveInvoiceByInvoiceID", ctx, mockDB, mock.Anything).Once().Return(invoiceIssued, nil)
				mockDB.On("Begin", ctx).Once().Return(mockTx, nil)
				mockPaymentRepo.On("GetLatestPaymentDueDateByInvoiceID", ctx, mockTx, mock.Anything).Once().Return(payment, nil)
		...
		{
			name:         "failed test - InvoiceRepo.Update DB Error",
			ctx:          interceptors.ContextWithUserID(ctx, ctxUserID),
			req:          voidInvoiceReq,
			expectedResp: failedResp,
		...
		{
			name:         "failed test - InvoiceActionLogRepo.Create DB Error",
			ctx:          interceptors.ContextWithUserID(ctx, ctxUserID),
			req:          voidInvoiceReq,
			expectedResp: failedResp,
		...
		{
			name:         "failed test - InvoiceBillItemRepo.FindAllByInvoiceID DB Error",
			ctx:          interceptors.ContextWithUserID(ctx, ctxUserID),
			req:          voidInvoiceReq,
			expectedResp: failedResp,
		...
		{
			name:         "failed test - BillItemRepo.FindByID DB Error",
			ctx:          interceptors.ContextWithUserID(ctx, ctxUserID),
			req:          voidInvoiceReq,
			expectedResp: failedResp,
		...
		{
			name:         "failed test - UpdateBillItemStatus failed",
			ctx:          interceptors.ContextWithUserID(ctx, ctxUserID),
			req:          voidInvoiceReq,
			expectedResp: failedResp,
		...
		{
			name:         "failed test - UpdateBillItemStatus partially failed with bill items",
			ctx:          interceptors.ContextWithUserID(ctx, ctxUserID),
			req:          voidInvoiceReq,
			expectedResp: failedResp,
	...
	for _, testCase := range testcases {
		t.Run(testCase.name, func(t *testing.T) {
			...
			response, err := s.VoidInvoiceV2(testCase.ctx, testCase.req.(*invoice_pb.VoidInvoiceRequestV2))
			...
			mock.AssertExpectationsForObjects(t, mockDB, mockInvoiceRepo, mockPaymentRepo, mockBillItemRepo, mockInvoiceBillItemRepo, mockInvoiceActionLogRepo, mockBulkPaymentRepo)
		})
	}
}
