...
import (
	...
	generator "github.com/manabie-com/backend/internal/invoicemgmt/services/payment_request_generator"
...
func (s *InvoiceModifierService) CreatePaymentRequest(ctx context.Context, req *invoice_pb.CreatePaymentRequestRequest) (*invoice_pb.CreatePaymentRequestResponse, error) {
...
func (s *InvoiceModifierService) processPaymentRequestFile(
	ctx context.Context,
	...
	stringNormalizer, err := utils.NewStringNormalizer()
	if err != nil {
		return err
	}
	...
	basePaymentGenerator := &generator.BasePaymentRequestGenerator{
		...
		BillItemRepo:                      s.BillItemRepo,
		InvoiceAdjustmentRepo:             s.InvoiceAdjustmentRepo,
	...
	enableFormatPaymentRequestFileFields, err := s.UnleashClient.IsFeatureEnabled(constant.EnableFormatPaymentRequestFileFields, s.Env)
	if err != nil {
		return status.Error(codes.Internal, fmt.Sprintf("%v unleashClient.IsFeatureEnabled err: %v", constant.EnableFormatPaymentRequestFileFields, err))
	}

	enableEncodePaymentRequestFiles, err := s.UnleashClient.IsFeatureEnabled(constant.EnableEncodePaymentRequestFiles, s.Env)
	if err != nil {
		return status.Error(codes.Internal, fmt.Sprintf("%v unleashClient.IsFeatureEnabled err: %v", constant.EnableEncodePaymentRequestFiles, err))
	}
	...
	case invoice_pb.PaymentMethod_CONVENIENCE_STORE.String():
		...
		enableBillingMessageInCSVMessages, err := s.UnleashClient.IsFeatureEnabled(constant.EnableBillingMessageInCSVMessages, s.Env)
		if err != nil {
			return status.Error(codes.Internal, fmt.Sprintf("%v unleashClient.IsFeatureEnabled err: %v", constant.EnableBillingMessageInCSVMessages, err))
		}
		...
		paymentRequestGenerator = &generator.CScsvPaymentRequestGenerator{
			...
			EnableBillingMessageInCSVMessages:        enableBillingMessageInCSVMessages,
			EnableFormatPaymentRequestFileFields:     enableFormatPaymentRequestFileFields,
			EnableEncodePaymentRequestFiles:          enableEncodePaymentRequestFiles,
		}
...
func (s *InvoiceModifierService) validateGeneratePaymentRequest(req *invoice_pb.CreatePaymentRequestRequest) error {
	...
	case invoice_pb.PaymentMethod_CONVENIENCE_STORE.String():
		...
		areThereEmptyDueDates := req.ConvenienceStoreDates.DueDateFrom == nil || req.ConvenienceStoreDates.DueDateUntil == nil

		// If there are empty due dates and the KEC Feedback Flag is off return error else do not continue to validation of due dates
		if areThereEmptyDueDates {
