...
func (s *InvoiceModifierService) DownloadBulkPaymentValidationsDetail(ctx context.Context, req *invoice_pb.DownloadBulkPaymentValidationsDetailRequest) (*invoice_pb.DownloadBulkPaymentValidationsDetailResponse, error) {
	...
	if len(listBulkPaymentValidationsDetails) == 0 {
		return nil, status.Error(codes.Internal, fmt.Sprintf("error no associated bulk payment validation detail records"))
	}
...
func (s *InvoiceModifierService) generateImportedPaymentValidationDetail(ctx context.Context, bulkPaymentValidationsDetails *entities.BulkPaymentValidationsDetail, paymentMethod string) (*invoice_pb.ImportPaymentValidationDetail, error) {
