...
func validatePartnerCSData(partnerCS *entities.PartnerConvenienceStore) error {
	if len(strconv.Itoa(int(partnerCS.ManufacturerCode.Int))) != 6 {
		return status.Error(codes.Internal, "The manufacturer_code of the partner CS should be 6 digits")
	}

	if len(strconv.Itoa(int(partnerCS.CompanyCode.Int))) != 5 {
		return status.Error(codes.Internal, "The company_code of the partner CS should be 5 digits")
	}
