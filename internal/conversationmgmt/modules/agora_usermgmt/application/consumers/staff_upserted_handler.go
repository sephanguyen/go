...
// Handle for case:
// + Create staff
type StaffUpsertedHandler struct {
	DB               database.Ext
...
// Note: this handle will be received upsert event (containing create and update)
func (h *StaffUpsertedHandler) Handle(ctx context.Context, value []byte) (bool, error) {
	ctx, cancel := context.WithTimeout(ctx, 5*time.Second)
	defer cancel()

	evtUpsertStaff := &upb.EvtUpsertStaff{}
	err := proto.Unmarshal(value, evtUpsertStaff)
	if err != nil {
		h.Logger.Error(err.Error())
		return false, err
	}

	// Ignore all update staff event
	if evtUpsertStaff.Type == upb.EvtUpsertStaff_UPSERT_STAFF_TYPE_UPDATE {
		return false, nil
	}

	agoraUser, err := mappers.UpsertStaffEvtToAgoraUserEnt(evtUpsertStaff)
	if err != nil {
		return false, fmt.Errorf("err mappers.UpsertStaffEvtToAgoraUserEnt: %w", err)
	}
	...
	err = try.Do(func(attempt int) (bool, error) {
		...
		if err != nil {
			...
			if attempt < constants.TimesRetryChatVendorRequest {
				...
				return true, fmt.Errorf("cannot create Agora user [staff]: [%v]", err)
			}

			return false, fmt.Errorf("cannot create Agora user [staff]: [%v]", err)
		}
	...
	if err != nil {
		...
		if errConvert != nil {
			return false, fmt.Errorf("error create Agora user [staff]: [%v], error get Agora user failure [staff] entity: [%v]", err, errConvert)
		}
		errCreateAgoraFailureUser := h.AgoraUserRepo.CreateAgoraUserFailure(ctx, h.DB, agoraUserFailure)
		if errConvert != nil {
			return false, fmt.Errorf("error create Agora user [staff] on Agora server: [%v], error create Agora user [staff] failure in DB: [%v]", err, errCreateAgoraFailureUser)
		}
	}

	err = h.AgoraUserRepo.Create(ctx, h.DB, agoraUser)
	if err != nil {
		return false, fmt.Errorf("error create Agora user [staff] in DB: [%v]", err)
	}
