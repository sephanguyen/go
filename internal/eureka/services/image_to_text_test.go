package services

import (
	"context"
	"errors"
	"testing"
	"time"

	"github.com/manabie-com/backend/internal/golibs/mathpix"
	mock_mathpix "github.com/manabie-com/backend/mock/golibs/mathpix"
	epb "github.com/manabie-com/backend/pkg/manabuf/eureka/v1"

	"github.com/stretchr/testify/assert"
	"github.com/stretchr/testify/mock"
	"google.golang.org/grpc/codes"
	"google.golang.org/grpc/status"
)

func TestImageToText_DetectFormula(t *testing.T) {
	t.Parallel()
	ctx, cancel := context.WithTimeout(context.Background(), time.Second)
	defer cancel()

	mockMathpixFactory := &mock_mathpix.Factory{}
	f := &ImageToText{
		MathpixFactory: mockMathpixFactory,
	}

	testCases := []TestCase{
		{
			name: "err wrong content format",
			ctx:  ctx,
			req: &epb.DetectFormulaRequest{
				Src: "wrong content",
			},
			expectedErr: status.Errorf(codes.InvalidArgument, "invalid src"),
			setup: func(ctx context.Context) {
				// wrong input content will throw error
			},
		},
		{
			name: "err DetectTextFromImage",
			ctx:  ctx,
			req: &epb.DetectFormulaRequest{
				Src: "data:image/png;base64,",
			},
			expectedErr: status.Errorf(codes.Internal, "MathpixFactory.DetectLatexFromImage: something went wrong"),
			setup: func(ctx context.Context) {
				mockMathpixFactory.On("DetectLatexFromImage", ctx, mock.Anything).Once().Return(nil, errors.New("something went wrong"))
			},
		},
		{
			name: "blank image DetectTextFromImage",
			ctx:  ctx,
			req: &epb.DetectFormulaRequest{
				Src: "data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAEMAAAAICAYAAACiVULcAAAAAXNSR0IArs4c6QAAADBJREFUSEtj/P///3+GUQAOAcbRwECkhNHAQMoVo4ExGhjYC8nRlDGaMkZTBsEGBACgBh/p2xTzwwAAAABJRU5ErkJggg==",
			},
			setup: func(ctx context.Context) {
				mockMathpixFactory.On("DetectLatexFromImage", ctx, mock.Anything).Once().Return([]mathpix.Data{}, nil)
			},
		},
		{
			name: "happy case",
			ctx:  ctx,
			req: &epb.DetectFormulaRequest{
				Src: "data:image/png;base64,",
			},
			setup: func(ctx context.Context) {
				mockMathpixFactory.On("DetectLatexFromImage", ctx, mock.Anything).Once().Return([]mathpix.Data{}, nil)
			},
		},
	}

	for _, testCase := range testCases {
		t.Run(testCase.name, func(t *testing.T) {
			testCase.setup(testCase.ctx)
			req := testCase.req.(*epb.DetectFormulaRequest)
			_, err := f.DetectFormula(testCase.ctx, req)
			if testCase.expectedErr != nil {
				assert.Equal(t, testCase.expectedErr.Error(), err.Error())
			} else {
				assert.Equal(t, testCase.expectedErr, err)
			}
		})
	}
}
