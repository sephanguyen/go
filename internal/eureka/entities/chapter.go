...
func (rcv *Chapter) FieldMap() (fields []string, values []interface{}) {
	fields = []string{"chapter_id", "name", "country", "subject", "grade", "display_order", "school_id", "updated_at", "created_at", "deleted_at", "copied_from", "current_topic_display_order", "book_id"}
	values = []interface{}{&rcv.ID, &rcv.Name, &rcv.Country, &rcv.Subject, &rcv.Grade, &rcv.DisplayOrder, &rcv.SchoolID, &rcv.UpdatedAt, &rcv.CreatedAt, &rcv.DeletedAt, &rcv.CopiedFrom, &rcv.CurrentTopicDisplayOrder, &rcv.BookID}
	return
}
