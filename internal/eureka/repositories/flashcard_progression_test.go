...
func TestFlashcardProgressionRepo_Upsert(t *testing.T) {
	...
	validFlashcardProgressionsReq := []*entities.FlashcardProgression{
		{
	...
	testCases := []TestCase{
		{
			name:        "happy case",
			req:         validFlashcardProgressionsReq,
			expectedErr: nil,
			setup: func(context.Context) {
				var fields []interface{}
				for i := 0; i < len(validFlashcardProgressionsReq); i++ {
					_, field := validFlashcardProgressionsReq[i].FieldMap()
		...
		},
		{
			name:        "exec error",
			req:         validFlashcardProgressionsReq,
			expectedErr: fmt.Errorf("eureka_db.BulkUpsertFlashCardProgression error: exec error"),
			setup: func(context.Context) {
				var fields []interface{}
				for i := 0; i < len(validFlashcardProgressionsReq); i++ {
					_, field := validFlashcardProgressionsReq[i].FieldMap()
		...
		},
		{
			...
			req:         validFlashcardProgressionsReq,
			expectedErr: fmt.Errorf("eureka_db.BulkUpsertFlashCardProgression error: no row affected"),
			setup: func(context.Context) {
				var fields []interface{}
				for i := 0; i < len(validFlashcardProgressionsReq); i++ {
					_, field := validFlashcardProgressionsReq[i].FieldMap()
	...
	for _, testCase := range testCases {
		...
		testCase.setup(ctx)
		err := flashcardProgressionRepo.Upsert(ctx, db, testCase.req.([]*entities.FlashcardProgression))
...
func TestFlashcardProgressionRepo_Get(t *testing.T) {
	...
	testCases := []TestCase{
		{
			...
			req: &GetFlashcardProgressionArgs{
				StudySetID:      database.Text("study-set-id"),
				StudentID:       database.Text("student-id"),
				LoID:            database.Text("lo-id"),
				StudyPlanItemID: database.Text("study-plan-item-id"),
				LmID:            database.Text("lm-id"),
			...
			setup: func(ctx context.Context) {
				e.StudySetID.Set("id")
				mockDB.MockQueryArgs(t, nil, mock.Anything, mock.AnythingOfType("string"),
					database.Text("study-set-id"),
					database.Text("student-id"),
					database.Text("lo-id"),
					database.Text("study-plan-item-id"),
					database.Text("lm-id"),
					database.Text("study-plan-id"),
					database.Int8(1).Get(),
					database.Int8(2).Get(),
				)
		...
		},
		{
			...
			expectedErr: fmt.Errorf("err db.Query: %v", pgx.ErrTxClosed),
		},
		{
			name: "err TxClosed without paging",
			req: &GetFlashcardProgressionArgs{
				StudySetID:      database.Text("study-set-id"),
				StudentID:       database.Text("student-id"),
				LoID:            database.Text("lo-id"),
	...
	for _, testCase := range testCases {
		ctx := context.Background()
		testCase.setup(ctx)
		req := testCase.req.(*GetFlashcardProgressionArgs)
		resp, err := repo.Get(ctx, mockDB.DB, req)
		if testCase.expectedErr != nil {
			assert.Equal(t, testCase.expectedErr.Error(), err.Error())
		} else {
			assert.Equal(t, testCase.expectedErr, err)
...
func TestFlashcardProgressionRepo_GetLastFlashcardProgression(t *testing.T) {
	...
	testCases := []TestCase{
		{
			...
			setup: func(ctx context.Context) {
				mockDB.MockQueryArgs(t, nil, mock.Anything, mock.AnythingOfType("string"), database.Text("study-set-id"), database.Text("lo-id"), database.Text("student-id"), database.Bool(true))
				e.StudySetID.Set("id")
				fields, values := e.FieldMap()
				mockDB.MockScanArray(nil, fields, [][]interface{}{values})
			},
		...
		},
		{
			...
			setup: func(ctx context.Context) {
				mockDB.MockQueryArgs(t, pgx.ErrTxClosed, mock.Anything, mock.AnythingOfType("string"), database.Text("study-set-id"), database.Text("lo-id"), database.Text("student-id"), database.Bool(true))
				fields, values := e.FieldMap()
				mockDB.MockScanArray(nil, fields, [][]interface{}{values})
			},
			expectedErr:  fmt.Errorf("err db.Query: %v", pgx.ErrTxClosed),
	...
	for _, testCase := range testCases {
		...
		resp, err := repo.GetLastFlashcardProgression(ctx, mockDB.DB, database.Text(req.StudySetID), database.Text(req.LoID), database.Text(req.StudentID), database.Bool(true))
		if testCase.expectedErr != nil {
			assert.Equal(t, testCase.expectedErr.Error(), err.Error())
		} else {
			assert.Equal(t, testCase.expectedErr, err)
