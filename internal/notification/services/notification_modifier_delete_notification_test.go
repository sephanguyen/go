...
func TestNotificationModifierService_DeleteNotification(t *testing.T) {
	t.Parallel()
	...
	testCases := []struct {
		Name  string
		Err   error
		Req   *npb.DeleteNotificationRequest
	...
	}{
		{
			...
			Setup: func(ctx context.Context) {
				...
				userNotificationRepo.On("SoftDeleteByNotificationID", ctx, mockTx, es[0].NotificationID.String).Once().Return(nil)
				...
				questionnaireUserAnswerRepo.On("SoftDeleteByQuestionnaireID", ctx, mockTx, []string{"questionnaire_id_1"}).Once().Return(nil)
				infoNotificationRepo.On("DiscardNotification", ctx, mockTx, database.Text("notification_id_1"), database.TextArray(nil)).Once().Return(nil)
	...
	for _, testCase := range testCases {
		t.Run(testCase.Name, func(t *testing.T) {
			...
			testCase.Setup(ctx)
			_, err := svc.DeleteNotification(ctx, testCase.Req)
			if testCase.Err == nil {
