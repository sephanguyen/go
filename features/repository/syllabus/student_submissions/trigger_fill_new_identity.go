...
func (s *Suite) insertAValidStudentSubmission(ctx context.Context) (context.Context, error) {
	stepState := utils.StepStateFromContext[StepState](ctx)
	...
	if _, err := s.DB.Exec(
		ctx,
		`
		INSERT INTO student_submissions (study_plan_item_id, assignment_id, student_id, student_submission_id, created_at, updated_at)
			VALUES($1, $2, $3, $4, $5, $5);
		`,
		database.Text(stepState.StudyPlanItemID),
		database.Text(stepState.AssignmentID),
		...
		return utils.StepStateToContext(ctx, stepState), fmt.Errorf("Insert student_submissions: %w", err)
	}
	return utils.StepStateToContext(ctx, stepState), nil
}

func (s *Suite) studentSubmissionNewIdentityFilled(ctx context.Context) (context.Context, error) {
	stepState := utils.StepStateFromContext[StepState](ctx)
