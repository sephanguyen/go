...
func (s *Suite) aUserInsertStudyPlanToDatabase(ctx context.Context) (context.Context, error) {
...
func (s *Suite) aUserCallStudyPlanOne(ctx context.Context) (context.Context, error) {
	stepState := utils.StepStateFromContext[StepState](ctx)

	variables := map[string]interface{}{
		"study_plan_id": graphql.String(stepState.StudyPlanID),
	}
	err := utils.QueryHasura(ctx, s.HasuraAdminURL, s.HasuraPassword, &stepState.StudyPlanOneQuery, variables)
	if err != nil {
		return utils.StepStateToContext(ctx, stepState), err
	}
	return utils.StepStateToContext(ctx, stepState), nil
...
func (s *Suite) ourSystemWillReturnStudyPlan(ctx context.Context) (context.Context, error) {
	stepState := utils.StepStateFromContext[StepState](ctx)

	if len(stepState.StudyPlanOneQuery.StudyPlanOne) == 0 {
		return utils.StepStateToContext(ctx, stepState), fmt.Errorf(" StudyPlanOne is empty: %d", len(stepState.StudyPlanOneQuery.StudyPlanOne))
	}
	if len(stepState.StudyPlanOneQuery.StudyPlanOne[0].StudyPlanItems) == 0 {
		return utils.StepStateToContext(ctx, stepState), fmt.Errorf(" StudyPlanItems is empty: %d", len(stepState.StudyPlanOneQuery.StudyPlanOne[0].StudyPlanItems))
	}

	if stepState.StudyPlanID != stepState.StudyPlanOneQuery.StudyPlanOne[0].StudyPlanID {
		return utils.StepStateToContext(ctx, stepState), fmt.Errorf("unexpected  StudyPlanID : want: %s, actual : %s", stepState.StudyPlanID, stepState.StudyPlanOneQuery.StudyPlanOne[0].StudyPlanID)
	}

	actualStudyPlan := stepState.StudyPlanOneQuery.StudyPlanOne[0].StudyPlanItems[0]
	if stepState.StudyPlanItemID != actualStudyPlan.StudyPlanItemID {
		return utils.StepStateToContext(ctx, stepState), fmt.Errorf("unexpected  StudyPlanItemID : want: %s, actual : %s", stepState.StudyPlanItemID, actualStudyPlan.StudyPlanItemID)
	}
	if stepState.AssignmentID != actualStudyPlan.AssignmentStudyPlanItem.AssignmentID {
		return utils.StepStateToContext(ctx, stepState), fmt.Errorf("unexpected  AssignmentID : want: %s, actual : %s", stepState.AssignmentID, actualStudyPlan.AssignmentStudyPlanItem.AssignmentID)
	}
	if stepState.AssignmentID != actualStudyPlan.ContentStructure.AssignmentID {
		return utils.StepStateToContext(ctx, stepState), fmt.Errorf("unexpected  AssignmentID : want: %s, actual : %s", stepState.AssignmentID, actualStudyPlan.ContentStructure.AssignmentID)
	}
	if stepState.TopicID != actualStudyPlan.ContentStructure.TopicID {
		return utils.StepStateToContext(ctx, stepState), fmt.Errorf("unexpected  TopicID : want: %s, actual : %s", stepState.TopicID, actualStudyPlan.ContentStructure.TopicID)
	}
	if stepState.LoID != actualStudyPlan.ContentStructure.LoID {
		return utils.StepStateToContext(ctx, stepState), fmt.Errorf("unexpected  LoID : want: %s, actual : %s", stepState.LoID, actualStudyPlan.ContentStructure.LoID)
	}
	if stepState.LoID != actualStudyPlan.LoStudyPlanItem.LoID {
		return utils.StepStateToContext(ctx, stepState), fmt.Errorf("unexpected  LoID : want: %s, actual : %s", stepState.LoID, actualStudyPlan.LoStudyPlanItem.LoID)
	}
	return utils.StepStateToContext(ctx, stepState), nil
}
