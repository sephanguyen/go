...
func (s *suite) aValidUser(ctx context.Context, opts ...userOption) (context.Context, error) {
	id := idutil.ULIDNow()
	ctx, err := s.aValidUserInDB(ctx, s.BobDBTrace, id, opts...)
	if err != nil {
		return ctx, err
	}
	ctx, err = s.aValidUserInDB(ctx, s.FatimaDBTrace, id, opts...)
	if err != nil {
		return ctx, err
	}
	return ctx, err
...
func (s *suite) aValidUserInDB(ctx context.Context, dbConn *database.DBTrace, randomID string, opts ...userOption) (context.Context, error) {
	stepState := StepStateFromContext(ctx)

	u := &entities.User{}
	database.AllNullEntity(u)
	firstName := fmt.Sprintf("valid-user-first-name-%s", randomID)
	lastName := fmt.Sprintf("valid-user-last-name-%s", randomID)
	...
	err := multierr.Combine(
		...
		u.PhoneNumber.Set(fmt.Sprintf("+848%s", randomID)),
		u.Email.Set(fmt.Sprintf("valid-user-%s@email.com", randomID)),
		u.Country.Set(cpb.Country_COUNTRY_VN.String()),
		u.Group.Set(constant.UserGroupAdmin),
		u.Avatar.Set(fmt.Sprintf("http://valid-user-%s", randomID)),
	)
	if err != nil {
		return StepStateToContext(ctx, stepState), err
	}
...
func (s *suite) HandleCreateUserGroup(ctx context.Context, tx *database.DBTrace, req *upb.CreateUserGroupRequest, resourcePath int) (*entity.UserGroupV2, error) {
	...
	err = userGroup.UserGroupID.Set(idutil.ULIDNow())
	if err != nil {
		return nil, fmt.Errorf("UserGroupID.Set: %w", err)
	}
	...
	if err = database.ExecInTx(ctx, tx, func(ctx context.Context, tx pgx.Tx) error {
		...
		for _, roleWithLocations := range req.RoleWithLocations {
			...
			err := grantedRole.GrantedRoleID.Set(idutil.ULIDNow())
			if err != nil {
				return fmt.Errorf("grantedRoleID.Set: %w", err)
			}
			// create granted_role
