...
func (s *suite) aValidStudentInDB(ctx context.Context, id string) (context.Context, error) {
	...
	err := multierr.Combine(
		...
		student.LegacyUser.FullName.Set(fmt.Sprintf("valid-user-%d", num)),
...
func (s *suite) createFirebaseUser(ctx context.Context, schoolID int32, user entity.LegacyUser) error {
