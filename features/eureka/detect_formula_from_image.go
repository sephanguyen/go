package eureka

import (
	"context"
	"fmt"
	"reflect"

	"github.com/manabie-com/backend/internal/golibs/mathpix"
	epb "github.com/manabie-com/backend/pkg/manabuf/eureka/v1"
)

type FormulasImgInfo struct {
	Src         string
	MathpixData []mathpix.Data
}

func (s *suite) aListOfFormulaImages(ctx context.Context) (context.Context, error) {
	stepState := StepStateFromContext(ctx)
	stepState.FormulasImgInfos = []*FormulasImgInfo{
		{
			Src: "data:image/png;base64,",
			MathpixData: []mathpix.Data{
				{
					Type:  "latex",
					Value: "\\frac{a-x}{\\left(a^{3}-x^{3}\\right)}",
				},
			},
		},
		{
			Src:         "data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAEMAAAAICAYAAACiVULcAAAAAXNSR0IArs4c6QAAADBJREFUSEtj/P///3+GUQAOAcbRwECkhNHAQMoVo4ExGhjYC8nRlDGaMkZTBsEGBACgBh/p2xTzwwAAAABJRU5ErkJggg==",
			MathpixData: []mathpix.Data{},
		},
	}

	return StepStateToContext(ctx, stepState), nil
}

func (s *suite) detectFormulaFromImages(ctx context.Context) (context.Context, error) {
	stepState := StepStateFromContext(ctx)
	result := make([]*FormulasImgInfo, 0, len(stepState.FormulasImgInfos))
	for _, info := range stepState.FormulasImgInfos {
		resp, err := epb.NewImageToTextClient(s.Conn).DetectFormula(s.signedCtx(ctx), &epb.DetectFormulaRequest{
			Src: info.Src,
		})
		if err != nil {
			return StepStateToContext(ctx, stepState), fmt.Errorf("internal errors: %w", err)
		}

		formulas := make([]mathpix.Data, 0, len(resp.Formulas))
		for _, item := range resp.Formulas {
			formulas = append(formulas, mathpix.Data{Type: item.Type, Value: item.Value})
		}
		result = append(result, &FormulasImgInfo{
			Src:         info.Src,
			MathpixData: formulas,
		})
	}
	stepState.Response = result
	return StepStateToContext(ctx, stepState), nil
}

func (s *suite) ourSystemMustReturnFormulasCorrectly(ctx context.Context) (context.Context, error) {
	stepState := StepStateFromContext(ctx)

	req := stepState.FormulasImgInfos
	resp := stepState.Response.([]*FormulasImgInfo)

	if !reflect.DeepEqual(req, resp) {
		return StepStateToContext(ctx, stepState), fmt.Errorf("formulas response is wrong")
	}

	return StepStateToContext(ctx, stepState), nil
}
