...
func init() {
	common.RegisterTest("virtualclassroom", &common.SuiteBuilder[common.Config]{
...
func setup(c *common.Config, fakeFirebaseAddr string) {
	...
	virtualClassroomSrvURL = "http://" + c.VirtualClassroomHTTPSrvAddr
	...
	err := connections.ConnectGRPC(ctx,
		...
		common.WithVirtualClassroomSvcAddress(),
		common.WithLessonMgmtSvcAddress(),
		common.WithUserMgmtSvcAddress(),
		common.WithVirtualClassroomHttpSvcAddress(c.VirtualClassroomHTTPSrvAddr),
	)
	if err != nil {
...
type suite struct {
	...
	*common.StepState
	...
	ApplicantID                string
	VirtualClassroomHTTPSrvURL string
	RootAccount                map[int]common.AuthInfo
}
...
func newSuite(c *common.Config) *suite {
	s := &suite{
		...
		VirtualClassroomHTTPSrvURL: virtualClassroomSrvURL,
	}
	...
	fs, err := filestore.NewMinIO(&c.Storage)
	if err != nil {
		zapLogger.Fatal(fmt.Sprintf("cannot init fire store: %v", err))
	}
	s.CommonSuite.FileStore = fs
