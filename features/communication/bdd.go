...
import (
	...
	noti_config "github.com/manabie-com/backend/internal/notification/config"
...
func init() {
	common.RegisterTest("communication", &common.SuiteBuilder[common.Config]{
...
func ScenarioInitializer(conf *common.Config, f common.RunTimeFlag) func(ctx *godog.ScenarioContext) {
...
func setup(c *common.Config, fakeFirebaseAddr string) {
	...
	mw := NewNotiSvcProxy{connections.NotificationMgmtConn}
	proxiedYasuo, err := grpc.DialContext(ctx, rsc.GetAddress("yasuo"), grpc.WithBlock(), grpc.WithTransportCredentials(insecure.NewCredentials()), grpc.WithUnaryInterceptor(
		grpc_middleware.ChainUnaryClient(mw.randomlyProxyToNewNotificationSVCMiddleware),
	))
	if err != nil {
		zapLogger.Fatal(fmt.Sprintf("failed to DialContext: %v", err))
	}
	proxiedBob, err := grpc.DialContext(ctx, rsc.GetAddress("bob"), grpc.WithBlock(), grpc.WithTransportCredentials(insecure.NewCredentials()), grpc.WithUnaryInterceptor(
		grpc_middleware.ChainUnaryClient(mw.randomlyProxyToNewNotificationSVCMiddleware),
	))
	connections.BobConn = proxiedBob
	connections.YasuoConn = proxiedYasuo
...
func initNotificationCommonState(ctx context.Context) context.Context {
	return noti_common.StepStateToContext(ctx, &noti_common.StepState{
		MapCourseIDAndStudentIDs: make(map[string][]string),
		MapStudentIDAndParentIDs: make(map[string][]string),
	})
}

func newNotificationCommonSuite(cfg *common.Config) *noti_common.NotificationSuite {
	csuite := &noti_common.NotificationSuite{}
	csuite.StepState = &noti_common.StepState{
		MapCourseIDAndStudentIDs: make(map[string][]string),
		MapStudentIDAndParentIDs: make(map[string][]string),
	}
	csuite.CommunicationHelper = helpers.NewCommunicationHelper(firebaseAddr, applicantID, connections, cfg)
...
func NewCommonSuite() *common.Suite {
	...
	commonSuite.StepState.FirebaseAddress = firebaseAddr
