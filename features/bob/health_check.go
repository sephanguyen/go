...
func (s *suite) healthCheckEndpointCalled(ctx context.Context) (context.Context, error) {
	stepState := StepStateFromContext(ctx)
	stepState.Response, stepState.ResponseErr = health.NewHealthClient(s.Conn).Check(contextWithValidVersion(ctx), &health.HealthCheckRequest{})
	return StepStateToContext(ctx, stepState), nil
}
func (s *suite) bobShouldReturnWithStatus(ctx context.Context, arg1, arg2 string) (context.Context, error) {
	stepState := StepStateFromContext(ctx)
	ctx, err := s.returnsStatusCode(ctx, arg1)
	if err != nil {
