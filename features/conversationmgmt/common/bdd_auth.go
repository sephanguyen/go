...
func (s *ConversationMgmtSuite) ContextWithToken(ctx context.Context, authtoken string) context.Context {
	return contextWithToken(ctx, authtoken)
}

func (s *ConversationMgmtSuite) ContextWithTokenAndTimeOut(ctx context.Context, token string) (context.Context, context.CancelFunc) {
	newCtx, cancel := context.WithTimeout(ctx, 60*time.Second)
	return contextWithToken(newCtx, token), cancel
}

func (s *ConversationMgmtSuite) generateExchangeToken(userID, userGroup string, orgID int64) (string, error) {
	firebaseToken, err := s.GenerateValidAuthenticationToken(userID, userGroup)
	if err != nil {
