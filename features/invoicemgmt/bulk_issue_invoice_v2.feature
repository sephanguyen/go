@quarantined
Feature: Bulk Issue Invoice Without Payment
  As an HQ manager or admin
  I can bulk change all "Draft" invoices to the status "Issued"

  Background:
    Given unleash feature flag is "disable" with feature name "BACKEND_Invoice_InvoiceManagement_ImproveBulkIssueInvoice"
  ...
  Scenario Outline: Admin bulk issue invoices successfully
    ...
    Examples:
      ...
      | school admin   | 1                 | BULK_ISSUE_DEFAULT_PAYMENT   | DIRECT_DEBIT           | DIRECT_DEBIT      | TODAY    | TODAY+1     | SCHEDULED    | BULK_PAYMENT_DEFAULT_PAYMENT   |
      | hq staff       | 2                 | BULK_ISSUE_DEFAULT_PAYMENT   | CONVENIENCE_STORE      | CONVENIENCE_STORE | TODAY+1  | TODAY+2     | MANUAL       | BULK_PAYMENT_DEFAULT_PAYMENT   |
      | hq staff       | 1                 | BULK_ISSUE_CONVENIENCE_STORE |                        | CONVENIENCE_STORE | TODAY+2  | TODAY+3     | MANUAL       | BULK_PAYMENT_CONVENIENCE_STORE |
