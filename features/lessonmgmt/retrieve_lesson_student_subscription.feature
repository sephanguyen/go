Feature: Admin Retrieve Student Subscriptions
    ...
    Scenario Outline: school admin retrieve student subscriptions with enrollment status
        ...
        Examples:
            | school | enrollment_status                   | limit | offset | total | rs_from_id | rs_to_id | next_offset | pre_offset | has_filter | keyword | courserIDs | grades | classIDs | locationIDs |
    ...
    Scenario Outline: school admin retrieve student subscriptions with lesson_date
        ...
        And a list of lesson_student_subscription_access_path are existed in DB
        When admin retrieve student subscriptions limit <limit>, offset <offset>, lesson date "<lesson_date>" with filter "", "", "", "" and ""
        Then returns "OK" status code
        And Bob must return list of <total> student subscriptions from "<rs_from_id>" to "<rs_to_id>" and page limit <limit> with next page offset "<next_offset>" and pre page offset "<pre_offset>" and around lesson date "<lesson_date>" and filter ""
