Feature: Admin Retrieve Lessons Management
  ...
  Background:
    ...
    And some centers
    And a course type with id "test-course-type-id" and school id 100
    And "enable" Unleash feature with feature name "Lesson_Student_UseUserBasicInfoTable"
    And "disable" Unleash feature with feature name "Lesson_Student_SearchInGivenNameColumn"
    ...
    Scenario Outline: school admin retrieve live lesson without filter
        ...
        When admin retrieve live lesson management tab "<lesson_time>" with page "<limit>" and "<offset>" on lessonmgmt
        Then returns "OK" status code
        And Lessonmgmt must return list of "<total>" lessons management from "<rs_from_id>" to "<rs_to_id>" and page limit "<limit>" with next page offset "<next_offset>" and pre page offset "<pre_offset>"
  ...
  Scenario Outline: school admin retrieve live lesson with filter
    ...
    When admin retrieve live lesson management tab "<lesson_time>" with filter "<key word>", "<date_range>", "<time_range>", "<teachers>", "<students>", "<courses>", "<center>", "<locations>", "<grade>", "<day_of_week>", "<scheduling_status>", "<classes>", "<grades_v2>", "test-course-type-id" on lessonmgmt
    Then returns "OK" status code
    And Lessonmgmt must return correct list lesson management tab "<lesson_time>" with "<key word>", "<date_range>", "<time_range>", "<teachers>", "<students>", "<courses>", "<center>", "<locations>", "<grade>", "<day_of_week>", "<scheduling_status>", "<classes>"
    ...
    Examples:
      ...
      | LESSON_TIME_FUTURE | student name | yes        | yes        | 0,1,2,3,4,5,6 | center-1 | teacher-1,teacher-2 | student-1,student-2 | courser-1 | 12,13 | center-2,center-3 | LESSON_SCHEDULING_STATUS_PUBLISHED                                    | class-1,class-2 |                 |
      ...
      | LESSON_TIME_FUTURE |              | yes        |            |               |          |                     |                     |           | 0     |                   | LESSON_SCHEDULING_STATUS_PUBLISHED,LESSON_SCHEDULING_STATUS_COMPLETED |                 | Grade-2         |
      ...
      | LESSON_TIME_PAST   | student name |            |            |               |          |                     |                     |           |       | center-1          |                                                                       |                 |                 |
      | LESSON_TIME_PAST   | student name |            |            |               |          |                     |                     |           |       |                   | LESSON_SCHEDULING_STATUS_CANCELED                                     |                 |                 |
      | LESSON_TIME_FUTURE | student name |            |            |               | center-1 |                     |                     |           |       |                   |                                                                       | class-1,class-2 |                 |

    # Scenario Outline: school admin retrieve live lesson with filter and Unleash turned on
    #     Given a signed in "school admin" with school: 100
    #     And "enable" Unleash feature with feature name "Lesson_Student_UseUserBasicInfoTable"
    #     And a list of lessons management of school 100 are existed in DB
    #     When admin retrieve live lesson management tab "<lesson_time>" with filter "<key word>", "<date_range>", "<time_range>", "<teachers>", "<students>", "<courses>", "<center>", "<locations>", "<grade>", "<day_of_week>", "<scheduling_status>", "<classes>", "<grades_v2>", "test-course-type-id" on lessonmgmt
