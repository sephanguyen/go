...
type userOptionInBob func(u *bob_entities.User)

func withIDInBob(id string) userOptionInBob {
	return func(u *bob_entities.User) {
		u.ID = database.Text(id)
	}
}

func withRoleInBob(group string) userOptionInBob {
	return func(u *bob_entities.User) {
		u.Group = database.Text(group)
	}
}

func withResourcePathInBob(resourcePath string) userOptionInBob {
	return func(u *bob_entities.User) {
...
func queryHasura(ctx context.Context, query interface{}, variables map[string]interface{}, withAdminPermission bool) error {
	...
	httpClient.Transport = rt
	client := graphql.NewClient(bobHasuraAdminUrl+graphqlPath, httpClient)
