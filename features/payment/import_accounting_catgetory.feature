Feature: Import accounting category
  Scenario Outline: Import valid csv file
    Given an accounting category valid request payload with "<row condition>"
    When "<signed-in user>" importing accounting category
    Then the valid accounting category lines are imported successfully
  ...
  Scenario Outline: Rollback failed import valid setting csv file with incorrect data
    Given an accounting category valid request payload with incorrect data with "<row condition>"
    When "<signed-in user>" importing accounting category
    Then the import accounting category transaction is rolled back
    And the invalid accounting category lines are returned with error
  ...
  Scenario Outline: Import invalid csv file
    Given an accounting category invalid "<invalid format>" request payload
    When "<signed-in user>" importing accounting category
