...
func (s *suite) aListOfLocationTypesInDB(ctx context.Context) (context.Context, error) {
	...
	for index, lt := range listLocationTypes {
		lt.locationTypeId += addedRandom
		lt.name += addedRandom
		if lt.parentLocationTypeId != "" {
			lt.parentLocationTypeId += addedRandom
		}
		stmt := `INSERT INTO location_types (location_type_id,name,parent_location_type_id,is_archived,updated_at,created_at,level) VALUES($1,$2,$3,$4,now(),now()) 
				ON CONFLICT ON CONSTRAINT location_types_pkey DO NOTHING`
		_, err := s.BobDBTrace.Exec(ctx, stmt, lt.locationTypeId,
			lt.name,
			NewNullString(lt.parentLocationTypeId),
			lt.archived, index)
		if err != nil {
			return StepStateToContext(ctx, stepState), fmt.Errorf("cannot insert locations with `id:%s`, %v", lt.locationTypeId, err)
		}
		if lt.expected {
			stepState.LocationTypeIDs = append(stepState.LocationTypeIDs, lt.locationTypeId)
		}
	}
...
func (s *suite) retrieveLocationTypes(ctx context.Context) (context.Context, error) {
	stepState := StepStateFromContext(ctx)
	req := &mpb.RetrieveLocationTypesRequest{}
	stepState.Response, stepState.ResponseErr = mpb.NewMasterDataReaderServiceClient(s.MasterMgmtConn).RetrieveLocationTypes(contextWithToken(s, ctx), req)
...
func (s *suite) mustReturnCorrectLocationTypes(ctx context.Context) (context.Context, error) {
	stepState := StepStateFromContext(ctx)
	rsp := stepState.Response.(*mpb.RetrieveLocationTypesResponse)
	...
	expectedLocationTypeIDs := stepState.LocationTypeIDs
