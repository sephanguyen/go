...
where
	case when start_date IS NOT null and end_date IS NOT null then
		 date_trunc('week', start_date::Date) <= scprs.week_start and scprs.week_start <= (date_trunc('week', end_date::Date) + interval '6 day')
		else
			CASE WHEN start_date IS NOT NULL THEN scprs.week_start >= date_trunc('week', start_date::Date) ELSE true END
			and CASE WHEN end_date IS NOT NULL THEN scprs.week_end <= (date_trunc('week', end_date::Date) + interval '6 day') ELSE true end
		end
...
WHERE
  CASE WHEN start_date IS NOT NULL AND end_date IS NOT NULL THEN
		date_trunc('week', start_date::Date) <= scps.student_start_date and scps.student_start_date <= (date_trunc('week', end_date::Date) + interval '6 day')
		ELSE
			CASE WHEN start_date IS NOT NULL THEN scps.student_start_date >= date_trunc('week', start_date::Date) ELSE true END
      AND CASE WHEN end_date IS NOT NULL THEN scps.student_end_date <= (date_trunc('week', end_date::Date) + interval '6 day') ELSE true END
	END
