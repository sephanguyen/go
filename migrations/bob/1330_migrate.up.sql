-- add constraint in granted_permission table
ALTER TABLE granted_permission DROP CONSTRAINT IF EXISTS granted_permission__uniq;
ALTER TABLE granted_permission ADD CONSTRAINT granted_permission__uniq UNIQUE (user_group_id, role_id, permission_id, location_id);

-- function to retrieve user_group data from 6 tables
CREATE OR REPLACE FUNCTION retrieve_src_granted_permission(userGroupID text) RETURNS SETOF public.granted_permission 
SECURITY INVOKER
AS
$$
BEGIN
RETURN QUERY(SELECT ug.user_group_id, ug.user_group_name, r.role_id, r.role_name, p.permission_id, p.permission_name, grap.location_id, ug.resource_path 
	FROM user_group ug
