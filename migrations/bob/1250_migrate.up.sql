...
from (select 
	scs.student_course_slot_id, 
	scs.student_id,
	scs.course_id, 
	scs.location_id, 
	scs.student_start_date,  
	scs.student_end_date, 
	scs.course_slot as "purchased_slot", 
	loc.created_at as "loc_created_at",
	loc.updated_at as "loc_updated_at"
	from student_course_slot scs
	inner join locations loc
	on loc.location_id = scs.location_id
	where scs.course_slot is not null
	and scs.student_end_date > current_date
	...
	left join (select 
		...
		on lm.user_id = scs.student_id
		inner join lessons l
		on l.lesson_id = lm.lesson_id and l.course_id = scs.course_id
		where l.scheduling_status != 'LESSON_SCHEDULING_STATUS_CANCELED'
		and scs.course_slot is not null
		and scs.student_end_date > current_date
		and scs.deleted_at is null
		and l.deleted_at is null
		and lm.deleted_at is null
		group by scs.student_course_slot_id, scs.course_id, scs.student_id) assigned_slot_info
on slot_info.student_course_slot_id = assigned_slot_info.student_course_slot_id
...
from (select 
	...
	scs.location_id, 
	generate_series(
		to_date(to_char(student_start_date, 'IYYY-IW'), 'IYYY-IW')
		,to_date(to_char(student_end_date, 'IYYY-IW'), 'IYYY-IW')
		,interval '1 week'
	)::date  as "week_start", 
	scs.course_slot_per_week as "purchased_slot", 
	loc.created_at as "loc_created_at",
	loc.updated_at as "loc_updated_at"
	from student_course_slot scs
	inner join locations loc
	on loc.location_id = scs.location_id
	where scs.course_slot_per_week is not null
	and scs.student_end_date > current_date
	and scs.deleted_at is null
	and loc.deleted_at is null) slot_info
	left join (select 
		...
		on l.lesson_id = lm.lesson_id and l.course_id = scs.course_id
		where l.scheduling_status != 'LESSON_SCHEDULING_STATUS_CANCELED'
		and scs.course_slot_per_week is not null
		and scs.student_end_date > current_date
		and scs.deleted_at is null
		and l.deleted_at is null
		and lm.deleted_at is null
		group by scs.student_course_slot_id, scs.course_id, scs.student_id, to_char(l.start_time, 'IYYY-IW')) assigned_slot_info
on slot_info.student_course_slot_id = assigned_slot_info.student_course_slot_id
