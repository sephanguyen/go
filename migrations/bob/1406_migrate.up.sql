...
DROP FUNCTION IF EXISTS public.student_course_purchased_recurring_slot_fn(keyword text, student_ids text[], course_ids text[], location_ids text[], timezone text, unique_id text, use_basic_info boolean);
DROP FUNCTION IF EXISTS public.student_course_assigned_recurring_slot_fn(keyword text, student_ids text[], course_ids text[], location_ids text[], timezone text, unique_id text, use_basic_info boolean);
DROP FUNCTION IF EXISTS public.student_course_recurring_slot_info_fn(keyword text, student_ids text[], course_ids text[], location_ids text[], start_date date, end_date date, timezone text, unique_id text, use_basic_info boolean);
...
group by CONCAT(sc.student_id, sc.course_id, sc.location_id, sc.student_package_id), sc.course_id, sc.student_id, to_char(l.start_time at time zone timezone, 'IYYY-IW')
;
$func$;
