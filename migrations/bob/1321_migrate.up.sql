...
CREATE OR REPLACE FUNCTION student_course_purchased_recurring_slot_fn()
RETURNS TABLE (
...
CREATE OR REPLACE VIEW public.student_course_purchased_recurring_slot AS
SELECT * FROM student_course_purchased_recurring_slot_fn();

-- RECURRING TAB - assigned slot
CREATE OR REPLACE FUNCTION student_course_assigned_recurring_slot_fn()
RETURNS TABLE (
    student_subscription_id text,
    course_id text,
    student_id text,
...
CREATE OR REPLACE VIEW public.student_course_assigned_recurring_slot AS
SELECT * FROM student_course_assigned_recurring_slot_fn();

-- RECURRING TAB - purchased and assigned slots calculation
CREATE OR REPLACE FUNCTION student_course_recurring_slot_info_fn()
RETURNS TABLE (
    student_id text,
    course_id text,
    location_id text,
...
scprs.resource_path
from student_course_purchased_recurring_slot scprs
left join student_course_assigned_recurring_slot scars
...
SELECT * FROM student_course_recurring_slot_info_fn();