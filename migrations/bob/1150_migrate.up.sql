DROP FUNCTION IF EXISTS get_previous_report_of_student_v3;

CREATE OR REPLACE FUNCTION public.get_previous_report_of_student_v3(report_user_id text, report_course_id text, report_id text, report_lesson_id text) 
    ...
    CASE WHEN report_id IS NOT NULL 
        THEN l.start_time < (
                select l1.start_time 
                    ...
                    where lr1.lesson_report_id = report_id and l1.deleted_at is NULL and lr1.deleted_at is NULL limit 1)
        ELSE l.start_time < (
                CASE WHEN report_lesson_id IS NOT NULL 
                    THEN
                        (select l2.start_time 
                            from lessons l2 where l2.lesson_id = report_lesson_id and l2.deleted_at is NULL limit 1)
                    ELSE now()
                END
            )
    END
