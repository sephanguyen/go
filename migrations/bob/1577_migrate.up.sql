DROP INDEX IF EXISTS users__username_gin__idx;
CREATE INDEX IF NOT EXISTS users__username_gin__idx ON public.users USING gin (username gin_trgm_ops);
...
CREATE OR REPLACE FUNCTION public.get_sorted_students_list_v3(
    ...
    AS $$
        WITH active_student_ids_with_grade_sequence AS (
            ...
            FROM get_active_student_ids_with_grade_sequence(location_ids, enrollment_statuses, grade_ids)
        ),
        sorted_students AS (
            SELECT u.*
            FROM users AS u
            ...
            WHERE (
                search_text IS NULL
                OR nospace(u.name) ILIKE nospace(search_text)
                OR nospace(u.full_name_phonetic) ILIKE nospace(search_text)
                OR u.email ILIKE search_text
                OR u.username ILIKE search_text
            )
