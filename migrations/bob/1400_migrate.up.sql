...
DROP FUNCTION IF EXISTS public.student_course_purchased_slot_fn(keyword text, student_ids text[], course_ids text[], location_ids text[], use_basic_info boolean);
DROP FUNCTION IF EXISTS public.student_course_assigned_slot_fn(keyword text, student_ids text[], course_ids text[], location_ids text[], use_basic_info boolean);
DROP FUNCTION IF EXISTS public.student_course_slot_info_fn(keyword text, student_ids text[], course_ids text[], location_ids text[], start_date date, end_date date, use_basic_info boolean);

-- SLOT TAB - purchased slot
CREATE OR REPLACE FUNCTION student_course_purchased_slot_fn(keyword text, student_ids text[], course_ids text[], location_ids text[], timezone text, unique_id text, use_basic_info boolean)
RETURNS TABLE (
    unique_id text,
    student_id text,
    course_id text,
...
base_slot_info.student_start_date,  
base_slot_info.student_end_date, 
...
from (select 
    ...
    sc.course_id, 
    sc.location_id,
    (sc.student_start_date at time zone timezone)::DATE as "student_start_date",  
    (sc.student_end_date at time zone timezone)::DATE as "student_end_date", 
...
CREATE OR REPLACE FUNCTION student_course_assigned_slot_fn(keyword text, student_ids text[], course_ids text[], location_ids text[], timezone text, unique_id text, use_basic_info boolean)
RETURNS TABLE (
    unique_id text,
    course_id text,
    student_id text,
