...
CREATE OR REPLACE FUNCTION student_course_purchased_slot_fn()
RETURNS TABLE (
...
CREATE OR REPLACE VIEW public.student_course_purchased_slot AS
SELECT * FROM student_course_purchased_slot_fn();

-- SLOT TAB - assigned slot
CREATE OR REPLACE FUNCTION student_course_assigned_slot_fn()
RETURNS TABLE (
    student_subscription_id text,
    course_id text,
    student_id text,
...
CREATE OR REPLACE VIEW public.student_course_assigned_slot AS
SELECT * FROM student_course_assigned_slot_fn();

-- SLOT TAB - purchased and assigned slots calculation
CREATE OR REPLACE FUNCTION student_course_slot_info_fn()
RETURNS TABLE (
    student_id text,
    course_id text,
    location_id text,
...
scps.resource_path
from student_course_purchased_slot scps
left join student_course_assigned_slot scas
...
SELECT * FROM student_course_slot_info_fn();
