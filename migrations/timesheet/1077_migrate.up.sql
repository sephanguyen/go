DROP FUNCTION IF EXISTS public.get_timesheet_count_v2;

CREATE
    OR REPLACE FUNCTION public.get_timesheet_count_v2(
    keyword text,
    from_date timestamp with time zone,
    to_date timestamp with time zone,
    location_ids_arg text[],
    staff_id_arg text
...
WHERE (t.deleted_at IS NULL)
  ...
  AND t.location_id = ANY(COALESCE(location_ids_arg, ARRAY[t.location_id]))
