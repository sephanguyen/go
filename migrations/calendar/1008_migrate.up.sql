...
CREATE POLICY rls_day_type ON "day_type" using (permission_check(resource_path, 'day_type')) with check (permission_check(resource_path, 'day_type'));
CREATE POLICY rls_day_type_restrictive ON "day_type" AS RESTRICTIVE TO PUBLIC using (permission_check(resource_path, 'day_type')) with check (permission_check(resource_path, 'day_type'));
ALTER TABLE "day_type" ENABLE ROW LEVEL security;
ALTER TABLE "day_type" FORCE ROW LEVEL security;
...
ALTER TABLE public.day_info ADD CONSTRAINT day_info_fk FOREIGN KEY (day_type_id,resource_path) REFERENCES public.day_type(day_type_id,resource_path);
