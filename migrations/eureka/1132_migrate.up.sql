-- Migrate to delete old data which are out of date before ALTER CONSTRAINT
DELETE FROM exam_lo_submission as main
 USING (SELECT submission_id,
               row_number() OVER (PARTITION BY shuffled_quiz_set_id ORDER BY updated_at DESC) AS row_number
          FROM exam_lo_submission) AS outdated
 WHERE main.submission_id = outdated.submission_id
   AND outdated.row_number > 1;

-- ALTER CONSTRAINT exam_lo_submission.shuffled_quiz_set_id
ALTER TABLE IF EXISTS public.exam_lo_submission
    DROP CONSTRAINT IF EXISTS shuffled_quiz_set_id_un,
...
-- Create trigger function for study_plan_items
DROP TRIGGER IF EXISTS trigger_study_plan_items_completed_at_to_exam_lo_submission_fn on public.study_plan_items;

CREATE OR REPLACE FUNCTION trigger_study_plan_items_completed_at_to_exam_lo_submission_fn()
    RETURNS TRIGGER
    LANGUAGE plpgsql
AS $FUNCTION$
BEGIN
...
DROP TRIGGER IF EXISTS trigger_study_plan_items_completed_at_to_exam_lo_submission_fn ON public.study_plan_items;
CREATE TRIGGER trigger_study_plan_items_completed_at_to_exam_lo_submission_fn
AFTER UPDATE OF completed_at ON public.study_plan_items
FOR EACH ROW
EXECUTE FUNCTION public.trigger_study_plan_items_completed_at_to_exam_lo_submission_fn();

-- Migrate missing data when study_plan_items.completed_at was updated to IS NOT NULL
INSERT INTO public.exam_lo_submission (
    submission_id,
    student_id,
    study_plan_id,
