...
select (learning_time.minute + sum(student_submissions.duration) / 60)::int,
...
from learning_time
         join student_submissions using (student_id)
where date = to_date(student_submissions.created_at::text, 'YYYY-MM-DD')
