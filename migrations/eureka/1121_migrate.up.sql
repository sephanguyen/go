...
BEGIN
    ...
    THEN
        ...
        VALUES (
            ...
            NEW.updated_at,
            NEW.deleted_at,
            (SELECT SUM(point) FROM public.quizzes WHERE external_id = ANY(NEW.quiz_external_ids)),
            NEW.resource_path
        )
        ON CONFLICT ON CONSTRAINT exam_lo_submission_pk DO UPDATE SET
            ...
            total_point = (SELECT SUM(point) FROM public.quizzes WHERE external_id = ANY(NEW.quiz_external_ids));
    END IF;
RETURN NULL;
END;
$FUNCTION$;
...
SELECT generate_ulid() AS submission_id,
       ...
       (SELECT SUM(point) FROM public.quizzes WHERE external_id = ANY(SQ.quiz_external_ids)) AS total_point,
