CREATE or replace FUNCTION public.get_list_course_student_study_plans_by_filter_v2(_course_id text, search text,
                                                                                   _book_ids text[], _status text,
                                                                                   _grades integer[]) RETURNS SETOF public.course_students
    LANGUAGE sql
    STABLE
AS
$$
...
FROM public.course_students as c_student
         LEFT JOIN student_study_plans_fn() as s_study_plans
