...
func GenRetrieveCourseStatisticV2Scenario(ctx context.Context, c *infras.ManabieJ4Config, dep *infras.Dep) (*j4.Scenario, error) {
	scenarioConf, err := c.GetScenarioConfig("Syllabus_RetrieveCourseStatisticV2")
	if err != nil {
		return nil, err
	}
	...
	scenarioOpt.TestFunc = func(ctx context.Context) error {
		...
		syllabusClient.RetrieveCourseStatisticV2(contextWithToken(ctx, token), req)
		return err
	}
	scenario, err := j4.NewScenario("Syllabus_RetrieveCourseStatisticV2", *scenarioOpt)
	if err != nil {
		return nil, err
	}
	return scenario, nil
